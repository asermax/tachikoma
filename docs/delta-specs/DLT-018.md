# Delta Spec: DLT-018 - Update core context files from conversation learnings
...
As part of this delta, the shared "prompt-driven forked processor" pattern (used by all memory extraction processors and this new one) is extracted into a DES document and the existing processors are refactored to use the generalized abstraction.

## Requirements

| ID | Requirement | Status |
|----|-------------|--------|
| R0 | Analyze completed conversations and update foundational context files (SOUL.md, USER.md, AGENTS.md) when clear evidence warrants it | Core goal |
| R1 | Detect user information changes (new job, location, projects, etc.) and update USER.md | Must-have |
| R2 | Detect personality/behavioral feedback ("be more concise", "stop using emojis") and update SOUL.md | Must-have |
| R3 | Detect operational instruction changes ("always use pytest", "prefer JSON over YAML") and update AGENTS.md | Must-have |
| R4 | Conservative update policy — only apply changes with clear conversational evidence; never overwrite correct info with noise | Must-have |
| R5 | Route updates to the correct context file based on information type | Must-have |
| R6 | Read-first strategy — read each file before modifying, preserving existing structure and merging changes contextually | Must-have |
| R7 | No-op when no context-file-relevant content in conversation | Must-have |
| R8 | Plug into the post-processing pipeline as a standard PostProcessor (main phase) | Must-have |
| R9 | Generalize the "prompt-driven forked processor" pattern into a DES document and refactor existing processors (episodic, facts, preferences) + this new one to use the abstraction | Must-have |
| R10 | Log what was updated for observability | Nice-to-have |
| R11 | Pending signals staging mechanism for ambiguous signals | Must-have |
| R11.1 | Structured format with entry date, stored at `.tachikoma/pending-signals.md` | Must-have |
| R11.2 | Agent is instructed to access pending signals only via provided read and add tools — no direct file modification or deletion; tool design should reinforce this constraint | Must-have |
| R11.3 | Processor auto-clears entries older than 1 month on every run (tunable threshold; 1 month chosen as a balance between giving signals enough time to recur across typical usage patterns while preventing unbounded growth) | Must-have |
| R11.4 | On recurrence detection, agent promotes the signal to a context file update; the original entry naturally ages out | Must-have |
| R12 | Clear boundary with memory extraction — context files are foundational identity documents (personality, user knowledge, operational instructions) while memory files are individual entries for retrieval; overlap in captured information is acceptable since they serve different purposes | Must-have |
...
### Pending Signals — Staging (R11, R11.1, R11.2)
...
- [ ] Given the agent interacts with the pending signals file, then it uses only the provided read and add tools — no direct file modification or deletion

### Pending Signals — Auto-Cleanup (R11.3)
...
### Boundary with Memory Extraction (R12)
...
### DES Pattern and Refactor (R9)

- [ ] Given the existing memory processors (episodic, facts, preferences) and the new context processor all follow the "prompt-driven forked processor" pattern, when the refactor is complete, then all four processors use a shared abstraction
- [ ] Given the refactored processors, when the post-processing pipeline runs, then behavior is identical to the pre-refactor state — all existing tests pass
- [ ] Given the DES document is created, then it describes the shared pattern: PostProcessor subclass with a prompt constant, forking the SDK session via `fork_and_consume()`, agent autonomously managing files

### Observability (R10)

- [ ] Given the processor updates a context file, when the update completes, then a log entry records which file was updated and a summary of the change
- [ ] Given the processor adds a pending signal, when the entry is added, then a log entry records the signal description

## Requires

Dependencies:
- DLT-007 (post-processing pipeline) — already implemented; this processor plugs into the existing pipeline infrastructure
...
- **docs/feature-designs/memory/memory-extraction.md** — Modifies: design updated to reference DES pattern instead of inline pattern description
- **docs/feature-designs/agent/post-processing-pipeline.md** — Adds: context processor in pipeline diagram, references to DES
- **docs/feature-specs/agent/workspace-bootstrap.md** — Modifies: pending-signals.md initialization may need a bootstrap hook or on-first-use creation
...
- DES document (e.g., `DES-004-prompt-driven-forked-processor.md`) needs to be created
- Memory extraction spec/design references to the fork pattern should point to the DES
- Pipeline spec/design should list the new processor alongside existing ones
