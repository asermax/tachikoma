...
| **S5** | telegramify-markdown formatter: `convert(markdown)` returns `(text, list[MessageEntity])` tuple → sent via `parse_mode=None` + `entities=[e.to_dict() for e in entities]`. Applied on each edit cycle. `split_entities()` available for splitting at 4096 UTF-16 code unit boundary with proper entity clipping. | |
...
| **S8** | `TelegramSettings` config section + bootstrap hook: optional `telegram: TelegramSettings | None` on Settings. `channel: Literal["repl", "telegram"] = "repl"` on root Settings. Bootstrap hook validates telegram config when `channel == "telegram"`, prompts for missing values (with guidance on obtaining chat ID), persists via SettingsManager. `_generate_default_config()` extended with commented `[telegram]` section. Hook is async (matching existing hook system) and calls `await bot.get_me()` directly for token validation. | |
| **S9** | Cyclopts CLI with SettingsManager override: `--channel` flag parsed by cyclopts, overrides `settings.channel` at runtime without persisting. SettingsManager's current `update(section, key, value)` API only supports section-level updates, so CLI override of the root-level `channel` field requires either extending `update()` with a root-level variant or mutating the tomlkit doc directly. SettingsManager is single source of truth. Cyclopts auto-generates `--help` output. | |
| **S10** | Graceful shutdown: aiogram natively handles SIGTERM/SIGINT via `loop.add_signal_handler`, setting an internal stop event. Use `@dp.shutdown()` hook to send partial response — this fires after polling stops but before bot session closes, so API calls still work. The `ResponseRenderer`'s buffer serves as the shared state for the partial response. In-flight handler tasks are NOT cancelled by aiogram, but the bot session closes shortly after, so the handler should catch connection errors gracefully. | |
...
Note: telegramify-markdown provides `split_entities(text, entities, max_utf16_len)` which splits
at newline boundaries and clips spanning entities across chunks. This can simplify steps 3-5
by operating on the already-formatted `(text, entities)` output. The paragraph-boundary preference
(step 3a) and tool-line handling (step 6) still need custom logic on top.
...
3. Apply CLI overrides at runtime (no save):
   └─ --channel value → override root-level channel in SettingsManager
   (SettingsManager.update() currently only supports section-level keys;
    need to extend with root-level support or mutate tomlkit doc directly)
...
6. bootstrap.run()
   └─ telegram_hook:
      ...
      d. Validate token via `await bot.get_me()` (hooks are async)
...
**Why**: Telegram's MarkdownV2 requires strict escaping and breaks with partial markdown during streaming (unclosed bold, incomplete code blocks). The entities-based approach sidesteps all parsing issues — `convert(markdown)` returns plain text + a list of `MessageEntity` objects, and the Telegram API applies formatting from entities directly. Safe for streaming: partial markdown produces valid plain text + whatever entities could be parsed. The library also provides `split_entities(text, entities, max_utf16_len)` for splitting at the 4096-char boundary with proper entity clipping across chunks.
**Sources**: telegramify-markdown v1.1.0 (March 2026), Telegram Bot API docs on MessageEntity, Telegram styled text entities docs
...
**Why This Over Alternatives**: telegramify-markdown is purpose-built for LLM output, built on pyromark (Rust pulldown-cmark bindings). `convert()` API returns `(text, list[MessageEntity])` with `to_dict()` for API serialization. Entity-based output confirmed compatible with both `send_message` and `edit_message_text` (parse_mode and entities are mutually exclusive in the API). A custom converter would need to handle all markdown edge cases. Plain text loses formatting during streaming.
**Consequences**:
...
- Pro: Safe for progressive edits — re-parsing the growing buffer each time produces correct entities
- Con: Additional dependency (~small, based on pyromark Rust bindings)
...
- Con: SettingsManager's `update(section, key, value)` only supports section-level updates. Root-level `channel` field requires either extending `update()` with a root-level overload, or directly mutating the tomlkit document for runtime overrides (no persist, so the approach is simpler)
...
**Then**: The `telegram_hook` validates the token via `await bot.get_me()` (hooks are async, matching the existing hook system). If it fails, startup aborts with a clear error message.
...
**Then**: aiogram's internal signal handler sets the stop event, ending the polling loop. The `@dp.shutdown()` hook fires (after polling stops, before bot session closes) — it reads the `ResponseRenderer`'s accumulated buffer and sends a final edit with the partial response. The in-flight handler task is not explicitly cancelled by aiogram, but the handler should catch connection errors gracefully since the bot session closes shortly after the shutdown hook returns. The process exits cleanly.
...
All previously open questions have been resolved through research:

- [x] **telegramify-markdown entities output** — Confirmed working. `convert(markdown)` returns `(text, list[MessageEntity])` tuples. Compatible with both `send_message` and `edit_message_text` via `entities=[e.to_dict() for e in entities]` with `parse_mode=None`. Safe for streaming: partial markdown produces valid plain text + whatever entities could be parsed. Library also provides `split_entities()` for 4096-char boundary splitting. (Source: telegramify-markdown v1.1.0, March 2026)
- [x] **aiogram `start_polling` graceful shutdown** — Confirmed working. aiogram registers signal handlers via `loop.add_signal_handler` for SIGTERM/SIGINT. The `@dp.shutdown()` hook fires after polling stops but before bot session closes — API calls still work there. Key nuance: in-flight handler tasks are NOT cancelled or awaited by aiogram; the handler should catch connection errors gracefully since the bot session closes shortly after. (Source: aiogram dispatcher source, issues #919, #1200, #1410)
- [x] **Async validation in bootstrap hook** — Premise was incorrect. The hook system is already async: `BootstrapHook = Callable[[BootstrapContext], Awaitable[None]]`, `Bootstrap.run()` is `async def` and awaits each hook. The `telegram_hook` can simply `await bot.get_me()` directly — no `asyncio.run()` or workarounds needed. All design references updated accordingly.
...
- telegramify-markdown v1.1.0: https://github.com/sudoskys/telegramify-markdown — key APIs: `convert()` for `(text, entities)`, `split_entities()` for chunking, `markdownify()` for MarkdownV2 (not used)
...
- `telegram_hook` is always registered but self-skips when `settings.channel != "telegram"` (idempotent, per DLT-023 hook convention). REPL-only usage triggers no Telegram validation or prompts. Hook is async, matching the existing hook system.
...
- aiogram shutdown behavior: in-flight handler tasks are NOT explicitly cancelled or awaited during `stop_polling()`. The `@dp.shutdown()` hook runs after polling stops but before bot session closes. If tighter control is needed, `handle_signals=False` + custom signal handling is available.
