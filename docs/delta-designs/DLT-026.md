...
**Sources**: SDK source code (`types.py` — `effort: Literal["low", "medium", "high", "max"]`).
**Options Researched**: Haiku (fastest, cheapest but lower quality), Sonnet (good balance), Opus with low effort (best quality with controlled latency).
**Why This Over Alternatives**: Opus with low effort provides the best classification quality for ambiguous edge cases while keeping response times fast. The effort parameter controls thinking budget, keeping latency comparable to smaller models for simple tasks.
**Consequences**:
...
- [x] ~~S2: Summary prompt design~~ → Incremental approach, Opus low effort, 5-8 sentence cap, "update" prompt pattern
- [x] ~~S5: Boundary detection prompt design~~ → JSON schema boolean output, Opus low effort, continuation-biased prompt
