...
| **S6** | Bridging context assembly and injection — query intermediate sessions via existing `get_by_time_range(previous_ended_at, now)`, exclude the matched session, filter those with summaries, concatenate chronologically (capped at `MAX_BRIDGING_SESSIONS` code constant, default 5), store on `_bridging_context`; `_build_options()` appends bridging section to system prompt when set (same one-time-then-clear pattern as `_previous_summary`) | |
...
| `src/tachikoma/sessions/repository.py` | New `get_recent_closed()` query; new `create_resumption()` and `get_resumptions_for_session()` methods; migration for `last_resumed_at` column on sessions table | `Base.metadata.create_all()` handles the new `session_resumptions` table automatically (it creates missing tables); `migrate()` gains a pragma check for the `last_resumed_at` column on the existing `sessions` table (same pattern as `summary` column) |
...
8. Assemble bridging context:
   ...
   c. Cap at MAX_BRIDGING_SESSIONS (code constant, default 5)
   d. Concatenate summaries chronologically
   e. self._bridging_context = assembled text (or None if empty)
...
- Pro: Requires a schema migration (new column), but follows the established `migrate()` pattern
...
**Then**: Only the most recent N (default ~5) intermediate session summaries are included, capped to avoid excessive prompt consumption. Sessions without summaries are excluded.
...
- **docs/feature-designs/agent/sessions.md** - Modifies: `Session` dataclass gains `last_resumed_at` field; `SessionRecord` gains column + migration; registry gains `reopen_session()`, `get_recent_closed()`, `record_resumption()` methods; Adds: `SessionResumption` dataclass, `SessionResumptionRecord` ORM model, `session_resumptions` table
...
- Sessions design needs `last_resumed_at` field documentation, `SessionResumption` model documentation, new registry methods (`reopen_session`, `get_recent_closed`, `record_resumption`), new migration documentation
...
- The candidate fetching query (`get_recent_closed`) runs on every message where boundary detection fires (active session + summary + cwd). The sessions table is small (at most thousands of rows) and the query filters on indexed `ended_at` with a narrow time window, so performance is negligible. If many sessions (20+) close within the window, the candidate list passed to the LLM should be capped (e.g., 10 most recent) to limit prompt size and token consumption.
- The bridging context cap (`MAX_BRIDGING_SESSIONS`, default 5) prevents prompt bloat when many short sessions occurred between the original and resumed session. This is a code constant, not user-configurable — it bounds the system prompt growth without adding configuration surface area.
...
- Schema migration strategy: `Base.metadata.create_all()` (called in `repository.initialize()`) automatically creates the new `session_resumptions` table since it creates missing tables. The `migrate()` method gains an additional pragma check for the `last_resumed_at` column on the existing `sessions` table, following the same pattern as the `summary` column migration.
