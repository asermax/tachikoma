...
- Channels (`telegram`, `repl`): gain event bus consumption for proactive session task delivery
...
│          ▼                 ▼                  ▼                          │
│  ┌──────────────┐  ┌───────────────┐  ┌──────────────┐                 │
│  │ Task         │  │ asyncio.Queue │  │ Coordinator-  │                 │
│  │ Repository   │  │ (event bus)   │  │ like executor │                 │
│  │ (SQLite)     │  │               │  │ (SDK client)  │                 │
│  └──────────────┘  └───────┬───────┘  └──────────────┘                 │
│                            │                                            │
│                   ┌────────┴─────────┐                                  │
...
| **S5** | Session task scheduler — periodic async loop (~5 min) that queries pending session instances, idle-gates via `last_message_time`, and pushes ready tasks onto an `asyncio.Queue` (event bus) for channel consumption | |
| **S6** | Session task channel integration — channel listens on the shared `asyncio.Queue` alongside user messages via `asyncio.wait` multiplexing. On dequeue: marks instance running via repository callback, calls `coordinator.send_message()` with the task prompt, renders the response, marks instance completed. Concurrent user messages during task processing are handled as steers. Telegram: queue consumption task runs alongside aiogram polling. REPL: `prompt_async()` and `queue.get()` wrapped in `asyncio.Task`s with `asyncio.wait(return_when=FIRST_COMPLETED)` | |
...
| **S9** | Transient instance generation — on background task completion (with non-null `notify`) or failure, create a session task instance with null definition_id carrying the notification content. Auto-cleanup from DB after successful delivery | |
...
| **S11** | Bootstrap hook — `tasks_hook` in `tasks/hooks.py`: initializes task DB tables via Alembic `upgrade head`, starts instance generator, session task scheduler, and background task runner as `asyncio.Task`s, marks any previously-running instances as failed (crash recovery). Returns the `asyncio.Queue`, task references, and repository via `bootstrap.extras`. Graceful shutdown: `__main__.py` `finally` block cancels all three async tasks and the runner marks in-progress instances as failed | |
...
| `src/tachikoma/tasks/scheduler.py` | `InstanceGenerator` — async loop evaluating definitions; `SessionTaskScheduler` — async loop pushing ready tasks onto queue; both accept `TaskRepository` and config | Generator and scheduler are plain async functions started as `asyncio.Task`s |
| `src/tachikoma/tasks/executor.py` | `BackgroundTaskRunner` — async loop picking up pending background instances; `BackgroundTaskExecutor` — manages a single background task's SDK session lifecycle with evaluator loop; uses `asyncio.Semaphore` for concurrency | Extracts coordinator-like SDK client management for multi-turn background sessions |
| `src/tachikoma/tasks/hooks.py` | `tasks_hook` — bootstrap hook (DES-003): creates DB, initializes repository, starts async loops, crash recovery; stores references in `bootstrap.extras` | Subsystem-owned hook; idempotent |
...
sequenceDiagram
    ...
    loop every ~5 min
        ...
        alt idle window exceeded
            ...
            Sched->>Queue: put((instance, completion_callback))
            Queue-->>Channel: get() (awaiting)
            Channel->>Coord: send_message(task.prompt)
            Coord-->>Channel: AgentEvent stream
            Channel-->>Channel: render response to user
            Channel->>Channel: call completion_callback()
        else user is active
        ...
        loop until complete/failed/max_iterations
            ...
            alt complete
                ...
                opt notify field set
                    Exec->>Repo: create transient session instance
                end
            else not complete
                Exec->>SDK: query(evaluator_feedback) [resume]
            else stuck/failed
                Exec->>Repo: mark failed
                Exec->>Repo: create transient session instance (failure notification)
            end
        end
    end
...
- Channel ↔ Event bus: `asyncio.Queue` consumed alongside user messages
...
- Bootstrap ↔ Task subsystem: `tasks_hook` initializes DB, starts async loops, stores references in `extras`
...
- Background task execution errors: instance marked `failed`, failure notification queued
- Queue delivery errors: logged, instance stays pending for retry
...
- **`asyncio.Queue`** (event bus): shared between `SessionTaskScheduler` (producer) and channels (consumer). Created in bootstrap, passed via `extras`.
...
3. For each instance:
   ...
   c. If idle: mark instance running, push (instance, completion_callback) onto asyncio.Queue
4. Channel's event loop (select between user input and queue):
   a. Dequeue (task_instance, callback) tuple
   b. Call coordinator.send_message(instance.prompt)
   c. Consume and render AgentEvent stream
   d. If user sends a message during processing: treated as steer()
   e. Call completion_callback() — marks instance completed in repository
...
3. For each instance (gated by asyncio.Semaphore, max_concurrent=3):
   ...
   f. If complete and notify is set: create transient session instance
   g. If failed: create transient session instance with failure message
4. Sleep until next tick
```
...
### Event bus (asyncio.Queue) for session task delivery

**Choice**: Use a shared `asyncio.Queue` as the communication channel between the session task scheduler and the active channel.
**Why**: The scheduler needs to deliver proactive messages through the channel without knowing which channel is active (Telegram or REPL). An async queue decouples the producer (scheduler) from the consumer (channel), enables future channels to integrate by simply consuming from the same queue, and avoids direct coupling between the task subsystem and channel implementations. The channel selects between user input events and the queue using `asyncio.wait` or equivalent multiplexing.
**Alternatives Considered**:
- Channel callback: scheduler calls a callback registered by the channel — tighter coupling, harder to test
- Coordinator-level injection: coordinator pushes events to channel via internal queue — adds responsibility to the coordinator that doesn't belong there

**Consequences**:
- Pro: Clean decoupling — scheduler has no channel dependency
- Pro: Future channels integrate by consuming from the same queue
- Pro: Easy to test — queue is a simple interface
- Con: Channel implementation must handle multiplexing between user input and queue
...
**Choice**: The `SessionTaskScheduler` marks instances as `running` when pushing them onto the queue, and provides a completion callback (closure over the repository) alongside the instance on the queue. The channel calls the callback after delivery — the channel never directly imports or uses `TaskRepository`.
**Why**: Channels should remain decoupled from the task subsystem. The scheduler owns the instance lifecycle and can provide a simple callback interface. The queue carries a `(TaskInstance, Callable)` tuple, keeping the channel's responsibility to just calling `send_message()` and invoking the callback.
**Alternatives Considered**:
...
**Consequences**:
- Pro: Channel stays decoupled — only knows about callbacks, not repositories
...
**Then**: The scheduler finds the pending session instance, checks `coordinator.last_message_time` (20 min ago > 5 min idle window), and pushes the instance onto the event bus queue. The channel dequeues it, calls `coordinator.send_message("Remind the user to check their emails...")`, and renders the agent's response via Telegram/REPL.
...
**Then**: A fresh `ClaudeSDKClient` is created with an adapted system prompt. The agent works on the task prompt. After each response, the evaluator assesses completion. When done, the instance is marked `completed` and a transient session task instance is created with the notification content. At the next scheduler tick (when idle), the notification is delivered via the event bus.
...
**Then**: The instance is marked `failed` with a reason. A transient session task instance is created: "Background task 'summarize-notes' failed: agent appeared to be stuck in a loop." This is delivered to the user at the next idle period.
...
**Then**: The evaluator forces a final assessment. If not complete, the instance is marked `failed`. A notification is queued for the user.
...
**Given**: A transient session task instance (null `definition_id`) was delivered successfully
**When**: The channel marks it as completed after delivery
**Then**: The instance is automatically deleted from the database (no retention for transient instances).
...
- **docs/feature-designs/agent/core-architecture.md** - Modifies: Coordinator gains `last_message_time` property and `mcp_servers` parameter on `_build_options()`; `__main__.py` wiring gains task tools server, event bus queue, and background task runner
...
- **docs/feature-designs/channels/telegram.md** - Modifies: Channel gains event bus consumption loop alongside user message handler
- **docs/feature-designs/channels/terminal-repl.md** - Modifies: REPL gains event bus consumption alongside input prompt
...
- Channel feature designs need event bus integration documentation
- Post-processing pipeline design needs note about separate instantiation for background tasks

## Notes

- The `asyncio.Queue` event bus is created in the bootstrap hook and passed to both the scheduler (producer) and channel (consumer) via `bootstrap.extras`. The channel receives it during wiring in `__main__.py`.
...
- The `asyncio.Queue` is unbounded by default. Backpressure is not a concern in practice: the instance generator only creates one pending instance per definition, and the scheduler only pushes when idle. If needed, a bounded queue with `maxsize` or periodic pruning can be added later.
- Graceful shutdown in `__main__.py`'s `finally` block: cancels the three async task loops (instance generator, session scheduler, background runner), the runner marks any in-progress background instances as `failed`, and the task repository is disposed (`close()`) alongside the session repository.
