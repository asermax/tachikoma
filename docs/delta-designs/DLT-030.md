...
- New `mcp_servers` parameter on `Coordinator` for exposing MCP tools to the main agent

**Wiring:** `__main__.py` creates the MCP server via `create_projects_server(workspace_path)` and passes it to the `Coordinator` constructor as `mcp_servers={"projects": projects_server}`. The coordinator forwards it to `ClaudeAgentOptions` on each message.
...
| **S3** | `register_project` MCP tool: runs `git submodule add <url> projects/<name>` via subprocess, resolves default branch from `git symbolic-ref refs/remotes/origin/HEAD`, checks out default branch; exposed to coordinator via `mcp_servers` | |
| **S3a** | Coordinator MCP support: add `mcp_servers` parameter to `Coordinator.__init__()`, pass through `_build_options()` to `ClaudeAgentOptions` | |
...
| **S5** | Projects context provider: extends `ContextProvider`, reads `projects/` submodules, returns `ContextResult(tag="projects", ...)` with names, paths, branches (or commit hash if detached); returns None if no projects | |
...
| `projects/context_provider.py` | `ProjectsContextProvider` pre-processor | Extends `ContextProvider`; pure filesystem reads, no agent spawn |
...
| `coordinator.py` | MCP server support | Add `mcp_servers` parameter (same type as `ClaudeAgentOptions.mcp_servers`: `dict[str, McpStdio/SSE/Http/SdkServerConfig]`) |
...
Output: ContextResult(tag="projects", content="<project list>") | None
```
...
**Submodule commit prompt**: The processor uses a dedicated `SUBMODULE_COMMIT_PROMPT` constant (not the workspace-specific `GIT_COMMIT_PROMPT`). The submodule prompt instructs the Haiku agent to: (1) run `git status` and `git diff` in the submodule, (2) group changes by purpose/directory, (3) create descriptive commits. Unlike the workspace prompt, it does **not** reference workspace-specific directories (`memories/`, `context/`). It uses generic code-change grouping logic (e.g., source files in one commit, configuration in another, tests in another).
...
### MCP Tools on Coordinator (New Pattern)

**Choice**: Add `mcp_servers` parameter to `Coordinator.__init__()`, pass through `_build_options()` to `ClaudeAgentOptions`.
**Why**: Register/deregister project tools must be available during live conversations, not just in forked post-processing sessions. The coordinator builds `ClaudeAgentOptions` per message — adding `mcp_servers` follows the same pass-through pattern as `agents`, `env`, and `allowed_tools`.
**Alternatives Considered**:
- No viable alternative: MCP tools are the SDK's mechanism for exposing custom tools; there's no other way to give the coordinator access to project management operations.

**Consequences**:
- Pro: Establishes a reusable pattern for future coordinator-level MCP tools (e.g., skills, task management)
- Con: First use of this pattern; needs thorough testing
...
- **docs/feature-designs/agent/core-architecture.md** - Modifies + Adds: register projects bootstrap hook, ProjectsProcessor in pre_finalize phase, ProjectsContextProvider in pre-processing, MCP server on coordinator
...
- **docs/feature-designs/agent/pre-processing-pipeline.md** - Adds: ProjectsContextProvider registered alongside MemoryContextProvider
...
- core-architecture: add `mcp_servers` param to Coordinator docs; document project management MCP tools
