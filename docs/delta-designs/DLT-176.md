...
- **Emoji extraction helper**: a module-level function `_emoji_set(reactions: list[ReactionTypeUnion]) -> set[str]` in `telegram/__init__.py` filters a list of `ReactionTypeUnion` (aiogram's discriminated-union alias on `MessageReactionUpdated.old_reaction`/`new_reaction`) to the emoji strings of `ReactionTypeEmoji` entries. Used twice in `_handle_reaction` (for old and new). Pure function; trivial to test in isolation.
