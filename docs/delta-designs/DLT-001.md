...
| **S5** | Terminal REPL: `Repl` class owns the input loop (`prompt_toolkit`'s `prompt_async()` with `FileHistory` at `~/.tachikoma/repl_history` and empty-input validator), delegates all output to a `Renderer` class that tracks line state (`_needs_newline`) and renders `AgentEvent`s to stdout (text chunks inline, tool activity as gray italic status lines with tool-specific details). `Renderer.render()` returns a bool: `True` to continue, `False` to exit (on non-recoverable errors). Handles Ctrl+C (exits REPL from prompt, interrupts stream during response), Ctrl+D (exits on empty prompt), and typed `exit`/`quit` commands | |
...
| `src/tachikoma/repl.py` | Terminal REPL (`Repl` for input, `Renderer` for output) | `Repl` owns input loop and control flow; `Renderer` owns event rendering and line state tracking. No SDK dependency |
...
**ToolActivity** carries `tool_name: str`, `tool_input: dict` and `result: str` — notifies the channel that the agent invoked a tool. The `tool_input` contains the tool's parameters (e.g., file path for Read, pattern for Grep, glob for Glob). The REPL displays this as a gray italic status line including relevant parameters (e.g., "Reading src/main.py...", "Searching for 'authenticate'...", "Globbing *.py...", "Running: ls -la", "Searching tools: select:Read"). Telegram might show it as a subtle indicator.
...
6. REPL delegates each AgentEvent to Renderer.render():
   - TextChunk → print inline, set _needs_newline flag
   - ToolActivity → newline if needed, print gray italic status line
   - Result → print newline, clear _needs_newline
   - Error → print to stderr, return False if non-recoverable
...
- The REPL separates input (`Repl`) from output (`Renderer`) concerns. The `Renderer` class encapsulates line-state tracking (`_needs_newline`) needed for correct newline insertion between text chunks and tool activity lines. This separation keeps the `Repl` class focused on control flow (input loop, exit conditions, interrupt handling) while making rendering independently testable
...
- `ClaudeSDKClient.query()` returns `None` — messages are retrieved via a separate `receive_messages()` call that yields an `AsyncIterator[Message]`
- SDK error types (`CLIConnectionError`, `CLINotFoundError`, `ProcessError`) are available from the public `claude_agent_sdk` namespace — avoid importing from the private `claude_agent_sdk._errors` module
- The `Message` union type includes `StreamEvent` alongside `UserMessage`, `AssistantMessage`, `SystemMessage`, and `ResultMessage` — the adapter filters `StreamEvent` the same way as other non-relevant types
- `AssistantMessage.error` is a `Literal["authentication_failed", "billing_error", "rate_limit", "invalid_request", "server_error", "unknown"] | None`
- Dev tooling (ruff, ty, pytest, just) follows ADR-001 through ADR-005 — same Astral toolchain as other projects
- **Test helpers pattern**: Plain helper functions (e.g., `make_assistant`, `make_result`) live in `tests/helpers.py`, not in `conftest.py`. `conftest.py` is reserved for pytest fixtures and hooks only. This avoids import issues with `--import-mode=importlib` and keeps concerns separated. Consider updating DES-001 to document this convention.
- **SDK message builders**: `tests/helpers.py` provides `make_assistant()` and `make_result()` as reusable builders for SDK message types used across test files. New SDK message builders should be added here as more test files need them.
