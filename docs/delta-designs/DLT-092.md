...
- `context/loading.py` (`SYSTEM_PREAMBLE_TEMPLATE`, `render_system_preamble`): timezone-aware preamble template and rendering
- `context/assembly.py` (`build_system_prompt`): accepts timezone for preamble rendering
- `coordinator.py`: stores timezone, passes to build_system_prompt
- `tasks/executor.py` (`BackgroundTaskExecutor`): injects current datetime into background task prompts
- `__main__.py`: plumbing timezone from settings to task tools server and Coordinator
...
| **S7** | Update `SYSTEM_PREAMBLE_TEMPLATE` Tasks section to include a `## Date and Time` subsection showing the configured timezone and a `TZ='{timezone}' date` command for current time lookup. Add `## Scheduling` documentation for bare ISO datetime interpretation. | |
| **S8** | Convert `SYSTEM_PREAMBLE` constant to `SYSTEM_PREAMBLE_TEMPLATE` with `{timezone}` placeholder. Add `render_system_preamble(timezone)` function that resolves timezone via `get_timezone()` and formats the template. Remove `load_context()` (deprecated wrapper). Update `build_system_prompt()` to accept `timezone` parameter and render preamble dynamically. | |
| **S9** | Plumb `timezone` from settings through `Coordinator.__init__()` to `build_system_prompt()` calls in `send_message()`. | |
| **S10** | In `BackgroundTaskExecutor._execute_background_task()`, resolve timezone via `get_timezone()` and inject a "Current date and time" line into the system prompt before `BACKGROUND_TASK_SYSTEM_PROMPT`. | |
| **S11** | Rename `_get_timezone()` to `get_timezone()` (public) for reuse across scheduler, preamble rendering, and executor. | |
...
| `src/tachikoma/context/loading.py` (`SYSTEM_PREAMBLE_TEMPLATE`) | Timezone-aware template for system preamble | Template string with `{timezone}` placeholder; `render_system_preamble()` resolves and formats |
| `src/tachikoma/context/loading.py` (`render_system_preamble`) | Renders preamble with resolved timezone | Calls `get_timezone()` for resolution chain, formats template |
| `src/tachikoma/context/assembly.py` (`build_system_prompt`) | Accepts `timezone` kwarg, renders preamble dynamically | Passes timezone to `render_system_preamble()` |
| `src/tachikoma/coordinator.py` | Stores and passes timezone to `build_system_prompt` | New `timezone` init parameter stored as `_timezone` |
| `src/tachikoma/tasks/executor.py` (`BackgroundTaskExecutor`) | Injects current datetime into background task system prompts | Resolves tz via `get_timezone()`, formats `datetime.now(tz)` |
| `src/tachikoma/tasks/scheduler.py` (`get_timezone`) | Public timezone resolution function (renamed from `_get_timezone`) | Reused across scheduler, preamble rendering, and executor |
...
### Scenario: System preamble renders with configured timezone

**Given**: `tasks.timezone` is configured to `"America/Argentina/Buenos_Aires"`
**When**: The system prompt is assembled for a conversation turn
**Then**: `build_system_prompt(entries, timezone="America/Argentina/Buenos_Aires")` renders the preamble containing `**America/Argentina/Buenos_Aires**` and `TZ='America/Argentina/Buenos_Aires' date '+%A, %B %d, %Y at %H:%M:%S %Z (%z)'`
**Rationale**: The agent needs to know its configured timezone and have a reliable way to get the current date and time

### Scenario: Background task gets current datetime injected

**Given**: `tasks.timezone` is `"America/Argentina/Buenos_Aires"` and current local time is `2026-04-05 14:30:00 ART`
**When**: A background task executes
**Then**: The system prompt starts with `Current date and time: Saturday, April 05, 2026 at 14:30:00 America/Argentina/Buenos_Aires` followed by `BACKGROUND_TASK_SYSTEM_PROMPT`
**Rationale**: Background tasks run in isolated sessions without conversation context — they need the current datetime for time-aware prompts
...
- **docs/feature-designs/tasks/task-management.md** - Modifies: task creation flow (tz plumbing to `create_task_tools_server`), `_parse_schedule` behavior (naive → tz-aware), `_format_schedule` behavior (converts to local tz), tool description content, component table for tools.py (new `timezone` param), background executor datetime injection
...
- **docs/feature-designs/agent/core-architecture.md** - Modifies: Coordinator accepts `timezone` parameter, system prompt assembly uses rendered preamble template instead of static constant

### Notes for Reconciliation
- task-management feature design: update task creation flow sequence diagram to show tz parameter; update component table for tools.py (new `timezone` param on factory); update _parse_schedule description in Data Flow section; add background executor datetime injection to the data flow
...
- core-architecture feature design: update Coordinator constructor to show `timezone` parameter; update system prompt assembly flow to show `build_system_prompt(entries, timezone=...)` with rendered preamble; note that `SYSTEM_PREAMBLE` static constant is replaced by `SYSTEM_PREAMBLE_TEMPLATE` + `render_system_preamble()`
...
- **Test impact**: The signature changes to `create_task_tools_server` (new `timezone` param), `_parse_schedule` (new `tz` param), `_format_schedule` (new `tz` param), `build_system_prompt` (new `timezone` kwarg), and `Coordinator.__init__` (new `timezone` param) require updates to existing tests. The `TestLoadContext` tests were replaced by `TestLoadContextIntegration` (using `load_foundational_context` + `build_system_prompt`) and `TestRenderSystemPreamble` (testing the new rendering function). The `test_preamble_import_is_correct` test was removed (no longer applicable — preamble is now a rendered template, not a shared constant).
