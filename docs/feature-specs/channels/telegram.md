...
**Steering path (mid-response)**: If the user sends another message (text or media) while a response is streaming, the channel checks `coordinator.in_exchange`. Because an SDK exchange is live, it calls `coordinator.enqueue()` directly — bypassing the delivery lock — and returns. The coordinator's forwarder moves the message onto the per-turn `sdk_inbox` and the message source yields it to the SDK as a steering message that influences the in-flight response, rather than being held back as a separate turn. (Buffered-delivery events from the priority buffer always start new exchanges via the lock and are never delivered as steering messages.)
