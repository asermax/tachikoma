...
**Acceptance Criteria**:
- Given the agent completes a tool while text is streaming, when the ToolActivity event arrives, then a tool status line (e.g., "> 🔧 Reading src/main.py") is appended to the current response message via edit
...
- Given tool execution finishes and text streaming resumes, when the first TextChunk arrives after tools, then the tool line is replaced with a dynamic summary of the tools that ran (e.g., "> 🔧 Reading 3 files and searching for 'config'") and new text continues below it in the same message
