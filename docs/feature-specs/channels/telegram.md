...
- As a user, I want the agent to see what message I'm replying to so that I can use references like "that one" or "yes, the second option" without repeating the original context
...
- Given `present_buttons` was called and buttons were successfully sent during the current response, when `notify()` runs, then neither copy nor delete is performed — the buttons message already delivered a push notification, and the main response message retains its original Telegram message ID (not replaced by a copy ID)
- Given `present_buttons` was called but returned an error (buttons not actually sent), when `notify()` runs, then copy+delete proceeds as normal (the skip only applies to successful sends)
- Given both `pin_message` and `present_buttons` were called during the same response, when `notify()` runs, then copy+delete is skipped (either condition is sufficient — the guards are independent)
...
When buttons are successfully sent during a response, the buttons message itself triggers a Telegram push notification. The `ResponseRenderer` skips its copy+delete notification for the main response message — the buttons message already delivered the notification, and preserving the original message ID maintains correct reply-to and reaction tracking. The skip only applies when the buttons send succeeded (validation errors and API failures do not trigger the skip).
...
### Reply-to Message Context (R19)

When a user replies to any Telegram message, the replied-to message's text is included as quoted context in the user's message sent to the agent. This gives the agent the context needed to understand references like "that one" or "yes, the second option" without requiring the user to repeat themselves.

The text is extracted from `reply_to_message.text` (for text messages) or `reply_to_message.caption` (for media messages with captions). When the replied-to message has no text or caption (stickers, voice messages, media without caption), no context is included. Long text is truncated: messages longer than 200 characters keep the first 100 and last 100 characters, joined by `[...]`; messages of 200 characters or fewer are included in full. The context is stored as the `message_prefix` field on the `TextMessage` envelope and assembled into `sdk_input` with a blank line separator. The formatting of the prefix content (e.g., `Replied to:\n> ...`) is owned by the Telegram channel; the envelope provides the generic prefix-to-text assembly.

This is independent from session routing (R18): reply-to context is included regardless of whether the reply triggers a session switch, and applies to all reply messages (text and media alike).

**Acceptance Criteria**:

Text extraction:
- Given a user replies to a text message, when the handler processes it, then the replied-to message's text is extracted and stored as `message_prefix` on the `TextMessage` envelope
- Given a user replies to a media message with a caption, when the handler processes it, then the caption is extracted and stored as `message_prefix`
- Given a user replies to a message with no text or caption (sticker, voice message, media without caption), when the handler processes it, then `message_prefix` is `None` and the agent receives only the user's text

Truncation:
- Given a replied-to message with text of 200 characters or fewer, when the handler processes it, then the full text is included in the quote block
- Given a replied-to message with text longer than 200 characters, when the handler processes it, then the text is truncated to the first 100 characters + `[...]` + last 100 characters

Rendering:
- Given a `TextMessage` with `message_prefix` set, when `sdk_input` is called, then the result is `{message_prefix}\n\n{user_text}`
- Given a `TextMessage` with `message_prefix` set to `None`, when `sdk_input` is called, then the result is the plain user text unchanged

Applicability:
- Given a reply to a message in the same session, when processed, then `message_prefix` is set on the envelope (reply context is independent of session routing)
- Given a non-reply message, when processed, then `message_prefix` is `None`
