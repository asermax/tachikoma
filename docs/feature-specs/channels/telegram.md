...
Text extraction:
- Given a user replies to a text message, when the handler processes it, then the replied-to message's text is extracted and stored as `replied_to_text` on the `TextMessage` envelope
- Given a user replies to a media message with a caption, when the handler processes it, then the caption is extracted and stored as `replied_to_text`
- Given a user replies to a message with no text or caption (sticker, voice message, media without caption), when the handler processes it, then `replied_to_text` is `None` and the agent receives only the user's text
...
Rendering:
- Given a `TextMessage` with `replied_to_text` set, when `sdk_input` is called, then the result is `Replied to:\n> {truncated_text}\n\n{user_text}`
- Given a `TextMessage` with `replied_to_text` set to `None`, when `sdk_input` is called, then the result is the plain user text unchanged

Applicability:
- Given a reply to a message in the same session, when processed, then `replied_to_text` is set on the envelope (reply context is independent of session routing)
- Given a non-reply message, when processed, then `replied_to_text` is `None`
