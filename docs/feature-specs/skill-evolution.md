...
| R3 | Per-branch data points are collected: which skills were invoked, which failed or were misapplied, what workarounds the agent resorted to, and which guidance or bundled tooling proved stale, redundant, or obsolete (a described workflow no longer exists; a CLI command or script is missing or broken) |
...
| R6b | An in-run maintenance pass consolidates near-duplicate patterns, empties pages superseded by the current skill — the skill no longer exists in the workspace, or its `SKILL.md` guidance or bundled files (the CLI command or script a merged proposal added, repaired, or removed) show the pattern was fixed — and sweeps emptied files |
