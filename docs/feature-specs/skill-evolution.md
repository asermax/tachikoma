...
- Given the proposal agent is seeded, then its input carries the impact log and excludes patterns with any impact-log entry (`proposed`, `accepted`, or `rejected`) from proposal candidates (the never-re-proposed rule is enforced input-side)
