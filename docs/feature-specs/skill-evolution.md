...
| R3 | Per-branch data points are collected: which skills were invoked, which failed or were misapplied, what workarounds the agent resorted to, and which guidance proved stale, redundant, or obsolete (the workflow it describes no longer exists) |
...
| R6b | An in-run maintenance pass consolidates near-duplicate patterns, empties pages whose skill no longer exists in the workspace (their pattern no longer applies), and sweeps emptied files |
| R7 | Proposals cover the full edit spectrum for workspace skills: adding, correcting, removing, or consolidating guidance in existing skills, creation of new skills, and — when a skill no longer serves a purpose — deletion of the skill's entire directory |
