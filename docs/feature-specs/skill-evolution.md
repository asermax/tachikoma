...
| ID | Requirement |
|----|-------------|
| R0 | The day's completed conversations are analyzed for skill usage, and recurring problems become reviewable, branch-based skill-change proposals — live skills are never edited silently |
...
| R3 | Per-branch data points are collected: which skills were invoked, which failed or were misapplied, what workarounds the agent resorted to (including a workflow no skill covers at all, improvised from scratch), and which guidance or bundled tooling proved stale, redundant, or obsolete (the same information restated in more than one file or step; a described workflow no longer exists; a CLI command or script is missing or broken) |
...
| R6b | An in-run maintenance pass consolidates near-duplicate patterns, empties pages superseded by the current skill — the skill no longer exists in the workspace, or its `SKILL.md` guidance or bundled files (the CLI command or script a merged proposal added, repaired, or removed) show the pattern was fixed; a page naming a workflow no skill covered is superseded only when a skill now covers that workflow, never merely because no skill of its name exists — and sweeps emptied files |
| R7 | Proposals cover the full edit spectrum for workspace skills: adding, correcting, removing, or consolidating guidance in existing skills; fixing or extending a skill's bundled tooling (a CLI command or script — with the tests the authoring guides require for bundled executables); creation of new skills; and — when a skill no longer serves a purpose — deletion of the skill's entire directory. A tooling gap is fixed in the tooling (guidance kept in sync), not papered over with documentation |
| R8 | Branch-per-change: a proposal agent authors each proposal on its own branch via a temporary worktree (change → commit → push to origin); the worktree and local branch are removed afterwards, and the live working tree is never touched. Each proposal is logged only after the host confirms from git state that the pushed branch exists (tip SHA read from git) — the proposal agent's report alone never creates or confirms a log entry |
...
| R17 | Authored proposals follow the house authoring conventions: the proposal run's skills are exactly the two built-in authoring guides (`skill-authoring`, `workflow-authoring`) — discovered from the bundled directory via the agent layer's `skillPaths` option and force-loaded into the run's input via `forceLoadSkills` — and the proposal system prompt requires every file the agent creates, edits, or deletes under the proposal worktree's `skills/` to conform to them (new skills follow the full `SKILL.md` conventions; new workflows use the step-directory format; edits preserve established structure, and removals leave the surviving skill coherent). Authored content follows the guides' content-placement and genericity rules as well — information lives in exactly one place with consumers pointing at it, conditionally-needed detail sits in references rather than always-loaded instructions, a placement violation in the edited content is fixed as part of the change rather than preserved (without reaching into unrelated parts of the skill), and pattern evidence motivates a proposal but never rides into the skill as dated, occurrence-specific content or examples. The guides' testing expectations for bundled executables are authoring conventions too: the run authors the tests and states how they run but cannot execute them (no shell tool). The rule is conditional on the guides' content being present in the run's input, so fail-soft loading and the policy never disagree; the guides are reference material, never proposal targets |
...
- Given a pattern page whose skill has been removed from the workspace tree (its removal proposal merged and pulled, or the skill deleted by hand), when the maintenance pass executes, then the page is emptied and its index line removed
- Given a pattern page whose pattern the current skill's guidance or bundled files resolve — a merged proposal added, repaired, or removed the CLI command or script the page indicts, and the page's entire content is superseded — when the maintenance pass executes, then the page is emptied and its index line removed
- Given a pattern page naming a workflow no skill covered (no skill directory for it ever existed), when the maintenance pass executes, then the page is emptied only when a skill now covers that workflow — the skill-no-longer-exists supersession rule does not retire it
...
**Acceptance Criteria**:
- Given accumulated evidence of a recurring skill problem with no impact-log entry for it, when the proposal agent runs, then it proposes at most one change for the pattern — adding to, correcting, removing, or consolidating the existing skill's guidance; fixing or extending its bundled tooling, with tests authored alongside (the run has no shell tool, so executing the tests belongs to the branch's review — test presence is prompt-enforced, never host-verified); a new `SKILL.md` for a recurring workflow that has no skill; or deletion of the skill's entire directory when the skill no longer serves a purpose — and may decline a pattern that does not justify a change (an empty report is valid)
- Given a pattern whose skill existed and is gone from the proposal worktree (each worktree is cut fresh from `origin/<default>` after the run's fetch — a removal proposal merged, or the skill deleted on the default branch), when the proposal agent runs, then it declines that pattern — propose nothing for it (prompt-enforced; the host verifies only scope)
...
- Given the proposal agent is running, when it creates, edits, or deletes files, then it may only touch files under `{workspace}/skills/` — built-in skills are never touched, and deleted paths count (a deletion-only diff is in scope; host-verified from the pushed diff)
...
- Given the proposal agent reports, then each proposal carries its reasoning alongside the one-line description — `problem`, `rootCause`, and `evidence`, restated from the acted-on pattern page and tailored to the authored change — and the full seven-field contract is stated on every surface the agent sees (the task prompt, the `report_proposals` schema and description, the system prompt's finishing section)
- Given a report entry whose skill, pattern, description, problem, rootCause, or evidence is empty or whitespace-only, when `report_proposals` is called, then the call is refused with an instructive message and nothing is captured (trim-then-test; all-or-nothing per call)
...
**Acceptance Criteria**:
- Given a run that created at least one verified proposal, when it completes, then an ad-hoc background task is dispatched (via the `task:dispatch-background` app event) with the configured post-work prompt — or the default notification-only prompt when none is configured, forwarded verbatim — and a `goal` naming the created proposals (so the background runner skips goal extraction), seeded with the run's context: proposals created, patterns touched, log path, and one review-ready block per verified proposal pairing its git facts with its reported reasoning (what the change does, the problem, the root cause, the evidence — the material a pull-request body should carry)
- Given the run context's proposal blocks, when they are rendered, then each block pairs facts and reasoning from the same reported entry (matched by branch; the first report of a duplicated branch, like verification), a proposal dropped at verification renders no reasoning, and no agent-authored line can restructure the context (single-line fields flatten; evidence renders as normalized bullets)
- Given the reported reasoning, then it rides the dispatch prompt only — the impact-log row stays the seven-column git-fact record, verification decisions stay git-only, and the framing over the blocks is informational (what the material is), never an instruction to open, merge, or edit anything
...
- Proposals are never auto-merged; the user merges or discards each branch. Whole-skill removal is proposed only from pattern evidence — the pass never proactively scans for dead skills
- Squash-merged proposals record as rejected — the change has landed, and rejected also blocks re-proposal
- Multiple open proposals may touch the same skill (different patterns); since each branch is cut from the same base, such proposals can conflict at merge time — an accepted risk of branch-per-change, not something the run prevents. A removal proposal conflicts by construction with any other open proposal for the same skill; whoever merges second resolves it
...
- A no-op diff (a pushed proposal whose branch changes nothing) passes the scope check vacuously and is logged — a conscious non-change; its row claims the pattern (blocking re-proposal even after the reviewer discards the empty branch) and counts as a verified proposal (firing the R10 dispatch)
