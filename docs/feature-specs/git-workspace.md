...
| R13 | The `scrub` tool removes given paths from the entire workspace history via `git filter-repo --invert-paths` and force-pushes to `origin`. Preconditions are checked in this order, each short-circuiting with its own result/message: empty path list (`NO_PATHS`), then dirty working tree (`DIRTY_TREE`), then `git filter-repo` not installed (`NOT_INSTALLED`), then paths absent from history (`PATHS_NOT_FOUND`). Because the dirty-tree check precedes the tool-availability check, a dirty repo missing filter-repo is rejected for the dirty tree, not the missing tool. Outcomes are surfaced as a result enum, never thrown |
