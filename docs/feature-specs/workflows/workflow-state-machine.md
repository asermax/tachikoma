...
| R34 | `update_workflow_state` accepts an optional `items: list[str]` parameter on the `start` action — required for loop steps and rejected for non-loop steps |
...
| R36 | Items are opaque strings; non-string items, non-list inputs, or missing items return validation errors before any state change is made |
...
- Given a non-loop step is being started with items provided, when `update_workflow_state(..., "start", items=[...])` is called, then the tool returns an error indicating items are not allowed for non-loop steps — the structural step-kind/items consistency check runs before condition evaluation, so this error surfaces even when the step's condition would otherwise fail
- Given items contains non-string values or is not a list, when `update_workflow_state` is called, then a Pydantic validation error is returned before the handler runs (atomic by construction)
