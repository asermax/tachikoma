...
| R18 | An authorized emoji reaction is surfaced to the agent as an inbound turn that names the emoji(s) added and/or removed; when the reacted-to message is older than its session's latest recorded message, the owning session's last exchange is prepended as a `Reacted to:` block with an interpretation-guidance suffix (a reaction to the latest message is left bare — already live); a reaction update whose emoji set is unchanged is dropped |
...
- Given the reacted-to message is older than its session's latest recorded message and the owning session has a `lastExchange`, when handled, then the turn text is prefixed with `Reacted to:\n<lastExchange>` and ends with `Interpret it in the context of the last exchange and respond accordingly.`
- Given the reacted-to message is its session's most recent recorded message (or no exchange is available), when handled, then no `Reacted to:` block or guidance is prepended — the turn text is the bare reaction prose
