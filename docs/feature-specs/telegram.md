...
| R28 | When a single streamed Telegram message accumulates more than `intensiveWorkThreshold` (default 4) tool→text boundaries — a contiguous block of one or more tool calls followed by any text counting as one boundary, not one per tool call — the turn is treated as intensive work and every intermediate segment (all baked `_🔧 <summary>_` markers and bridging text chronologically before the live streaming tail) folds into a single collapsed Telegram **expandable blockquote** (collapsed by default, tap-to-expand), leaving only the final substantive text expanded by default. The threshold is a trigger (collapse activates only once the count strictly exceeds it), not a quota of segments kept visible, so turns at or below it render exactly as R4. Collapse is positional — no length or content heuristic decides bridging vs. substantive — and the fold is retroactive (the live message is rebuilt from the buffer on each flush, so segments that had rendered inline fold in once the threshold is crossed). An in-progress tool stays expanded (a segment folds only once its tools complete, at the next boundary); detection resets at each Telegram message boundary so a split or already-committed message is never retroactively collapsed; `collapseIntensiveWork: false` (default on) disables it entirely. The `expandable_blockquote` is emitted by the renderer as a structured payload, not by the markdown converter (so `>` still maps to an expanded `blockquote`, R6) |
...
When a single streamed message crosses a boundary threshold, the renderer folds its accumulated intermediate activity into one collapsed Telegram expandable blockquote so the final answer is the prominent, expanded content by default — a path that replaces the markdown compose for that message once active, while every below-threshold and disabled turn renders byte-identically to R4.
...
- Given a turn whose final tool block has no following text, when it finalizes, then that trailing block adds no boundary (a boundary requires tool→text); its summary bakes at finalize into the collapsed block
...
- Given a turn crosses the threshold mid-stream, when the crossing boundary forms, then the next flush rebuilds the live message from the buffer so every segment before the live tail (including ones that had rendered inline) folds into the collapsed block, leaving only the live tail expanded
- Given collapse is active and a tool is currently running (its live `_🔧 <activity>_` line showing), when the renderer flushes, then the running tool remains expanded — it is the live tail; only completed segments fold, and a segment becomes foldable only when text or another tool resumes after it
- Given an intensive turn ending in a final answer, when it finalizes, then all intermediate segments are wrapped in a single collapsed `expandable_blockquote` and only the final substantive text is expanded by default; the block carries the baked `_🔧 <summary>_` markers (inner italic/code entities), not the live activity labels
- Given an intensive turn that ends on a tool with no following text, or whose threshold is crossed on its final boundary, when it finalizes, then the entire message is the collapsed block carrying the per-segment summaries — non-empty and expandable to reveal the work, never an empty message
- Given an intensive turn, when the finalized message is laid out, then the collapsed intermediate block renders above the final answer in chronological order, with the final answer as the last expanded content
