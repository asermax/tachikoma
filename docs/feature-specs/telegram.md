...
| R16 | Button taps are acknowledged immediately (before authorization), unpacked from the wire format, routed as a framed inbound turn carrying the tapped value; when the tapped button message is older than its session's latest recorded message, the original prompt text is prepended to the turn as context (a tap on the latest button message is left bare — the prompt is already live in the agent's context); single-use taps remove the keyboard via a detached call whose failure never blocks routing |
...
| R18 | An authorized emoji reaction is surfaced to the agent as an inbound turn that names the emoji(s) added and/or removed; unless the reaction targets the session's most recent message (already at the bottom of the conversation), the reacted-to message's text is recovered from its session-tree entry and prepended as a truncated `Reacted to:` quote so the agent knows which message the emoji targets (mirroring the reply quote, R21); a reaction update whose emoji set is unchanged is dropped |
| R19 | The channel records every message id it handles — inbound user messages and the bot's outbound replies — against the session that received or produced them in the `channel_messages` table, keyed by channel and message id (re-pointed on conflict). The most recently recorded message id for a session is derivable for recency checks, and an outgoing button prompt additionally stores its text so a later tap can recover it |
| R20 | When an inbound message (text, media, or reaction) targets a previously recorded message — via Telegram reply-to or the reacted-to message id — and that message maps to a known session, the turn carries `metadata.resumeSessionId` so the boundary force-routes it to the owning session; a target with no recorded session is handled by kind per R25 rather than silently falling through |
| R21 | A reply to a message that carries text quotes a truncated form of that text as a `Replied to:` prefix on the turn so the agent sees what was replied to, unless the replied-to message is its session's most recent recorded message (already live in the agent's context); routing metadata is set either way |
...
- Given the reaction targets a message other than the session's most recent one and its text is recovered, when handled, then the turn text is prefixed with `Reacted to:\n> <truncated text>` of that message and ends with `Interpret it in context and respond accordingly.`
- Given the reaction targets the session's most recent recorded message, when handled, then no `Reacted to:` block is prepended — the turn text is the bare reaction prose (the message is already at the bottom of the conversation)
...
- Given a reaction targets a message id that is not recorded, when handled, then a notice that the conversation couldn't be found is sent to the user and the reaction is not submitted (dropped rather than routed to an unrelated session); a notice send failure is warn-logged and the reaction is still not submitted
...
- Given a text/media reply targets a message id that is not recorded, when routed, then no `resumeSessionId` is attached, a system hint noting the reply couldn't be matched to its original conversation is appended to the turn, and it follows normal routing
...
- Given a reply targets a message that carries text, when mapped, then a truncated `Replied to:` quote of that text is prepended to the turn, unless the replied-to message is its session's most recent recorded message (already live in the agent's context) — in which case the quote is omitted but `metadata.replyToMessageId` is still set so routing is unaffected
...
- Given an authorized tap with recognized callback data (`btn1:`/`btnN:` prefix), when routed, then the turn text is `` The user tapped the option `<value>` out of the options you displayed. `` with `metadata.buttonValue`; when the tapped button message is older than its session's latest recorded message and was recorded with prompt text, that prompt is prepended (`<prompt>\n\n…`), while a tap on the latest button message is left bare (the prompt is already live)
