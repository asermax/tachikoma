...
| R6 | Messages are converted from the agent's GitHub-flavored markdown to MarkdownV2 (via `telegramify-markdown`) and sent with `parse_mode: "MarkdownV2"`; on a Telegram entity-parse or "message is too long" rejection the raw text is resent as plain text; other send errors propagate |
