...
| R6 | Messages are converted from the agent's GitHub-flavored markdown to MarkdownV2 (via `telegramify-markdown`) and sent with `parse_mode: "MarkdownV2"`; GFM tables are flattened to a bullet list first (the converter mishandles table cells into invalid MarkdownV2 and Telegram has no table rendering); on a Telegram entity-parse or "message is too long" rejection the raw text is resent as plain text; other send errors propagate |
...
- Given a response contains a GFM table (with any surrounding inline formatting), when converted, then the table is flattened to a bullet list (`- **<first cell>**: <remaining cells>`) before MarkdownV2 conversion, so the surrounding formatting is preserved and the cell content no longer triggers a parse rejection; tables inside fenced code blocks are left intact
