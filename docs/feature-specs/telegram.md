...
- Given a nightly (`closeTrunkIfDue`) trunk close with no exchange following, when post-processing runs, then no lead-in is created for the "Post-processing: …" lines — the coordinator suppresses them and logs only, since there is no imminent `respond()` to reclaim a lead-in and the user isn't watching (see [conversation-loop](conversation-loop.md) R20)
...
**Acceptance Criteria**:
- Given an exchange whose metadata carries a decision header, when `respond()` starts, then the renderer is seeded with the header before streaming begins, and it renders as a bold label plus one-line note anchored above the streamed text
