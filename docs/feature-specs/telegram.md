...
| R28 | When a single streamed Telegram message accumulates more than `intensiveWorkThreshold` (default 4) tool→text boundaries — a contiguous block of one or more tool calls followed by any text counting as one boundary, not one per tool call — the turn is treated as intensive work and every unit before the last content-type unit folds into a single collapsed Telegram **expandable blockquote** (collapsed by default, tap-to-expand), leaving the last unit expanded by default. A unit is a preface text segment plus the tool group it introduces; the collapse split therefore follows content-type transitions (it lands at the start of the last text segment, not at a paragraph break), so a streaming multi-paragraph text answer stays expanded as a whole rather than folding paragraph-by-paragraph. The threshold is a trigger (collapse activates only once the count strictly exceeds it), not a quota of segments kept visible, so turns at or below it render exactly as R4. Collapse is positional — no length or content heuristic decides what folds — and the fold is retroactive (the live message is rebuilt from the buffer on each flush, so units that had rendered inline fold in once the threshold is crossed). While a tool is running its preface text stays expanded together with the tool's live activity line (the preface explains the running tool group); a unit folds only at the next tool-group → new-text transition, and at `finalize()` everything before the last unit collapses (a turn ending on a trailing tool keeps its `[last preface + trailing tool summary]` expanded rather than collapsing the whole body). Detection resets at each Telegram message boundary so a split or already-committed message is never retroactively collapsed; `collapseIntensiveWork: false` (default on) disables it entirely. The `expandable_blockquote` is emitted by the renderer as a structured payload, not by the markdown converter (so `>` still maps to an expanded `blockquote`, R6) |
...
- Given a turn crosses the threshold mid-stream, when the crossing boundary forms, then the next flush rebuilds the live message from the buffer so every unit before the last content-type unit (including ones that had rendered inline) folds into the collapsed block, leaving the last unit expanded
- Given collapse is active and text is streaming within the last text segment, when successive paragraphs of that segment complete, then the whole segment stays expanded (not folded paragraph-by-paragraph); it folds only when a tool resumes after it
- Given collapse is active and a tool is currently running (its live `_🔧 <activity>_` line showing), when the renderer flushes, then the running tool's preface text stays expanded together with the tool's activity line as the last unit; only earlier units fold (the preface explains the running tool group)
...
- Given a long turn whose stream splits into multiple messages at the 4096-character limit, when each message is committed/streamed, then the boundary count is evaluated independently per message (detection resets at each split), an already-committed message is never retroactively collapsed, and a single committed chunk that itself exceeded the threshold carries its own collapsed block for its intermediate segments
