...
- Given the evaluator returns `complete`, then the instance is marked `completed`, the registered post-processors run once (git/projects persistence; transcript-dependent ones no-op), the final agent text is delivered idle-gated, and a `completed` status payload is emitted on the `notify` event
