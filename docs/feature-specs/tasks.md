...
| R9 | Pending background instances run through an iterative evaluator loop on a single persistent pi session opened once per execution, capped at `backgroundMaxIterations` (default 10); the session is prompted repeatedly across iterations (retaining its own history) and is disposed on every exit path; a background run uses the main conversational session's tool model (no hard tool allowlist) so its bound extension tools stay active alongside the built-in filesystem/bash tools and the `notify_user`/`ask_user` custom tools |
...
- Given the registered context providers yield blocks, when the run starts, then those blocks (workspace memory indexes, projects) are prepended to the opening prompt as `<context owner="…">` sections; later iterations need no re-injection because the persistent session retains its history
