...
| R8 | Pending session instances are transitioned to `running` (with `startedAt` stamped), then handed to channel delivery with `gate: "idle"` and a configurable max hold (`sessionTaskMaxHoldSeconds`, default 900); a successful handoff marks the instance `completed`, while a failed handoff rolls it back to `pending` (clearing `startedAt`) for retry on the next tick |
| R9 | Pending background instances run through an iterative evaluator loop in ephemeral in-memory pi sessions, capped at `backgroundMaxIterations` (default 10); a background run uses the main conversational session's tool model (no hard tool allowlist) so its bound extension tools stay active alongside the built-in filesystem/bash tools and the `notify_user`/`ask_user` custom tools |
...
| R9b | Before its first iteration a background run gathers the registered context providers' blocks (workspace memory indexes, projects) via `app.agent.collectContext` and prepends them to the task prompt in the live-session `<context owner="…">` format; the blocks are gathered once and reused across the loop's iterations |
...
| R12a | On background completion the registered post-processors run once via `app.sessions.runPostProcessors` (phase order, error-isolated) to persist workspace changes the task made (git commit/push, project state); transcript-dependent processors (episodic memory, core-context) no-op, since a background run persists no transcript |
| R13 | A background run pauses for user input by calling the `ask_user` tool: the instance transitions to `waiting`, the question and the agent's progress excerpt are persisted, the question is delivered to the user immediately (with the instance ID), a `waiting` status payload is emitted on the `tasks:instance-finished` event, and the run returns — releasing its concurrency slot |
...
**Acceptance Criteria**:
- Given a pending session instance, when the delivery pass runs, then it is first transitioned to `running` (with `startedAt` stamped), `app.channels.deliver` is called with the rendered task text (a "Scheduled task" label plus the prompt), `gate: "idle"`, `target: "agent"`, the configured max hold, and `metadata { kind: "session_task", instanceId }`, and on a successful handoff the instance is marked `completed`; the coordinator injects the `agent`-targeted delivery as a fresh turn the agent acts on
...
**Acceptance Criteria**:
- Given a pending background instance, when the runner dispatches it, then it is marked `running` and the agent runs with a background system prompt that includes the current date and time in the configured timezone, the `notify_user`/`ask_user` custom tools, the built-in filesystem/bash tools, and the curated background extension tools (skills + `delegate_to_agent`, git, projects, detached-processes, notifications, task tools) — with no hard allowlist filtering them out
- Given the registered context providers yield blocks, when the run starts, then those blocks (workspace memory indexes, projects) are prepended to the task prompt as `<context owner="…">` sections, gathered once and reused across iterations
...
- Given the evaluator returns `complete`, then the instance is marked `completed`, the registered post-processors run once (git/projects persistence; transcript-dependent ones no-op), the final agent text is delivered idle-gated, and a `completed` status payload is emitted on the `tasks:instance-finished` event
- Given the evaluator returns `error`, the iteration cap is reached, or the run throws, then the instance is marked `failed` and a failure notice is delivered alongside a `failed` status payload on the `tasks:instance-finished` event
