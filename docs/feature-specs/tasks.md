...
- Given a fresh background run with a null goal, when the run starts, then the executor derives a goal from the task prompt via structured extraction, stores it on the instance, and (when a parent definition still has no goal) writes it back so the next run reuses it without re-extracting
