...
| R13 | Four nightly maintenance jobs — one per memory type (episodic, facts, preferences) and one for context files — share a single configurable cron schedule, register as low-priority scheduler jobs, and run under the scheduler's shared low-priority semaphore (sequential by default, configurable via `[scheduler] max_concurrent_low`); can be disabled via an `enabled` toggle |
