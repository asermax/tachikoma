...
| R9 | `read_process_output` returns the tail of **both** the captured stdout and stderr logs by default, presented as two separated labeled sections (`[stdout]` then `[stderr]`, with an empty stream's section omitted) so the caller doesn't have to guess which stream a process writes to (many programs — Python `logging`, build tools, servers — write their useful output to stderr); `stream="stdout"` or `stream="stderr"` isolates a single stream. Output is trimmed with pi's `truncateTail` and prefixed with a truncation marker when shortened; when both streams are missing or empty it yields "No output yet." |
| R9a | `read_process_output` also supports a windowed read: when `offset` (0-based line, default 0) and/or `count` (lines, default 100) are given, it returns the `[offset, offset + count)` line slice instead of the tail — applied to the selected stream, or to **each stream in parallel** when both are read (the default), again as separated labeled sections with any stream whose window is past EOF omitted. A window starting at or past EOF returns a message naming the requested range and the log's line count (the longest log's count when both are read); a negative `offset` or `count` below 1 is rejected with a validation error |
...
**Acceptance Criteria**:
- Given `read_process_output` with only a `process_id`, then the tail of **both** the stdout and stderr logs is returned as separated `[stdout]`/`[stderr]` sections (an empty stream's section omitted); with `stream="stdout"` or `stream="stderr"`, only that stream's tail is returned
- Given both logs are empty or missing, then "No output yet." is returned
- Given output larger than pi's tail limits, then the response is truncated to the most recent output and prefixed with `[earlier output truncated]`
- Given `offset` and/or `count`, then the `[offset, offset + count)` line slice is returned (offsets 0-based) — of the selected stream, or applied to each stream in parallel as separated sections when both are read (a stream whose window is past EOF is omitted)
- Given a window that begins at or past the last line, then a message names the requested line range and the log's line count (the longest log's count when both are read) rather than returning empty content
