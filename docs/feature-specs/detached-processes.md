...
**Acceptance Criteria**:
- Given Tachikoma restarts and a `running` record's pid is dead, then the bootstrap reconciles it to `exited` with a `null` exit code and no notification is emitted
