...
Workflows map multi-step processes to directory trees inside skill packages: each workflow is a folder of ordered step directories, each step carrying an `instructions.md` with frontmatter. Running a workflow creates a database-persisted step state machine (pending/started/completed/skipped) that the agent drives through four lifecycle tools — start, update, query, end. Because state lives in the database and progress notes live in a scratchpad file, workflows survive context compaction and session boundaries; instances abandoned across sessions are expired by a cleanup post-processor. The extension also contributes a usage context section (scope: main only) explaining when to reach for a workflow and the compact driving model — top-level-id routing, instructions arriving in tool results, per-step-type mechanics, and the recovery entry point (issue-474); the full driving detail, the lifecycle tools, composition, loops, and stale-instance recovery live in its reference file (`references/workflows.md`, per [DES-014](../design/DES-014-two-tier-agent-facing-documentation.md)).

Steps can also compose other workflows. A step may declare `composes` (run a sub-workflow to completion, then resume the parent), `loop` (run a sub-workflow once per agent-supplied item), or `condition` (a natural-language predicate surfaced before the step starts — the agent decides whether to start or skip). Composition nests to arbitrary depth; the agent drives the whole nested run with a single top-level workflow ID, and a cascade engine resolves each step id against the active chain (deepest layer first), so an id the engine itself announced keeps working — including completing the in-flight composes/loop step of a waiting layer, which ends its sub-workflow early.
...
| R6 | `start_workflow` creates an instance with a unique ID and a scratchpad file, and returns the step list plus guidance (first-step call — or, when the first step carries a `condition`, a start-or-skip decision presenting the predicate and both calls, issue-471 — scratchpad usage, progression rules (covering every mechanics-bearing marker: `(loop: ...)` needs `items` on start, `(if: ...)` halts for an explicit start-or-skip decision, `(composes: ...)` runs a sub-workflow driven with the same id, and completing the in-flight step ends it early) — recovery, which points at re-reading the started step's instructions from the state view) (issue-474); it rejects unknown workflows, zero-step workflows, and a second instance while one is active for the same skill+workflow — the rejection cites the existing instance's ID and carries the compact recovery directive (R27) |
...
| R9 | `query_workflow` with an ID returns the full state view (skill, workflow, current step, scratchpad path, timestamps, and per-step states — each line carrying the same markers as the start list: `(skippable)`, `(if: ...)`, `(composes: ...)`, `(loop: ...)`, so a pending condition's predicate stays visible after context loss, issue-471); without an ID it lists all active workflows — the recovery path after context loss. The state view's current-step line names the step's `instructions.md` path so a started step's instructions can be re-read after context loss (issue-474) |
...
| R19 | A step may declare a `condition` (natural-language predicate); when the cascade auto-advances to it, auto-start halts, the step stays pending, and the response surfaces the predicate with instructions to `start` (passes) or `skip` (fails) — the agent is the evaluator. The shown start call is the literal next call: when the condition step also declares `loop`, it includes the `items` parameter (with the `items=[]` zero-iteration note). A `condition` makes an otherwise-required step skippable. The predicate is visible at every surface where a start decision is made — the auto-advance halt, the `start_workflow` first-step decision (R6), and the query-view markers (R9) — because skip is only valid while the step is pending (R8): the agent must be able to decide before committing to a start (issue-471) |
| R20 | When auto-advance reaches a pending loop step, it halts (the step stays pending) and the response prompts the agent to start it with `items`; the first-step guidance in `start_workflow`'s response likewise includes `items` when the workflow's first step is a loop step |
| R21 | The agent drives a nested run with the single top-level workflow ID; step ids resolve against the active chain deepest-layer-first, so an id the engine announced keeps working after a sub-workflow spawns (a colliding child id shadows the parent's). Completing the in-flight composes/loop step of a waiting layer ends its sub-workflow early: every layer below the owner is torn down, the step completes (loop bookkeeping frozen to fully iterated), and the owner auto-advances — every affected response notes which sub-workflows ended early. Any other transition on a suspended layer's step, and unknown ids, return an error naming the deepest active layer and its valid step ids |
...
| R27 | Agent-facing guidance: the already-active rejection (R6) directs inspection before ending; `start_workflow`'s prompt guidelines state the never-silently-discard rule; the `end_workflow` description directs surfacing what an instance the agent discovered (not one the user asked to stop) had done before ending it, noting both actions (`complete`/`abort`) discard the state and scratchpad identically; the reference page carries the canonical procedure (find → inspect → decide → surface, nested-stack teardown, staleness expiry); and the injected usage section teaches the compact driving model (top-level-id routing, instruction delivery, per-step-type mechanics, recovery entry) with full driving detail in the reference (issue-474) |
...
**Acceptance Criteria**:
- Given `start_workflow(skill_name, workflow_name)` for a valid workflow, when it runs, then a record is created with every step `pending`, a scratchpad file exists, and the response contains the step list (steps marked with their skippable/if/composes/loop markers), the workflow ID, and progression/recovery guidance
- Given the workflow's first step carries a `condition`, when `start_workflow` runs, then the Getting Started guidance replaces the first-step start directive with the condition decision — the predicate surfaced verbatim with start and skip calls naming the first step, identical to the halt's block except for the ordinal (first vs next) — while the step list, scratchpad note, progression, and recovery guidance are retained; a first step combining `condition` and `loop` announces the `items` call with the zero-iteration note, identical to the halt response for that combination (issue-471)
- Given a first step without a `condition`, when `start_workflow` runs, then the Getting Started guidance keeps the numbered start directive naming the first step (issue-471)
...
- Given `query_workflow(workflow_id)` on steps carrying frontmatter markers, when the view renders, then each step line carries its markers — the top-level, active-child, and child-ID standalone views alike (issue-471)
- Given `query_workflow(workflow_id)`, when the view renders (top-level, active-child, and standalone child alike), then the current-step line names the started step's `instructions.md` path — `none` before any start, bare id when the snapshot no longer resolves it (issue-474)
...
The tool definitions and the rejection directive are guarded by `tests/workflows/tools.test.ts`; the usage section and the reference page are pinned by the usage-sections content guard (`tests/usage-sections.test.ts`).
...
- Given the workflows usage section is injected, when read, then it teaches the driving model compactly — top-level-id routing, instructions arriving in the result that starts a step, per-step-type mechanics (`loop` items, `if` decisions, `composes` same-id driving), and the `query_workflow()` recovery entry — while the full driving detail (the reference's Driving a Workflow section, including the early-end mechanic) stays in the reference file (issue-474)
- Given the workflows usage section points at its reference file, when the reference page is read, then it carries the stale-instance recovery procedure — find (`query_workflow()`), inspect (the scratchpad, with the state view as evidence when the file is gone), decide (resume preferred; end only when superseded or unwanted), surface — plus the nested-stack teardown note and the `staleHours` expiry window, and a Driving a Workflow section covering instruction delivery, auto-advance, per-step-type start/skip mechanics, and the early-end mechanic (issue-474)
...
`runCascade` (`src/extensions/workflows/cascade.ts`) reads the active chain (top-level root down through composed/loop children), resolves the step id against the chain deepest-layer-first to find its owning layer, applies the transition there, and auto-advances across layer boundaries, staging every change as a single atomic `MutationBatch`. `validateWorkflowGraph` (`composition.ts`) runs at bootstrap to reject unsafe definitions.
...
- Given a halted condition step that also declares `loop`, when the agent issues the start call shown in the halt response (with `items`), then the call succeeds verbatim — the loop's first iteration begins (issue-467)
- Given an in-flight composes/loop step with a live sub-workflow below it, when the agent completes that step's id, then every layer below the owner is soft-deleted, the step completes (loop bookkeeping frozen to fully iterated, rendered as `N / N (complete)`), the owner auto-advances, and the response notes which sub-workflows ended early — on step-activation, halt, and finalize responses alike (issue-467)
- Given `start` or `skip` on an in-flight composition step, when attempted, then the call fails directing the agent to complete the step to finish it early (issue-467)
- Given a suspended layer's step that is not the in-flight one (valid or invalid transition), or an id unknown to every active layer, when attempted, then the call fails naming the deepest active layer and its valid step ids (issue-467)
...
- The stale-instance recovery path is also covered in the bundled `workflow-authoring` skill's Recovery section; the guide's conditional-step pattern directs authors to declare skip predicates as `condition` (surfaced before start) rather than body prose, which is readable only after the step starts (see [skills](skills.md))
