...
| R6 | `start_workflow` creates an instance with a unique ID and a scratchpad file, and returns the step list plus guidance (first-step call — or, when the first step carries a `condition`, a start-or-skip decision presenting the predicate and both calls, issue-471 — scratchpad usage, progression rules, recovery); it rejects unknown workflows, zero-step workflows, and a second instance while one is active for the same skill+workflow — the rejection cites the existing instance's ID and carries the compact recovery directive (R27) |
