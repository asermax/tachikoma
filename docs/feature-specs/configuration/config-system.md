...
- Given a config file with no `[agent]` section, when loaded, then `agent.model` defaults to `None` (SDK default), `agent.sub_agent_model` defaults to `"opus"` (sub-agent default), `agent.allowed_tools` defaults to `["Read", "Glob", "Grep"]`, `agent.cli_path` defaults to `None` (SDK bundled binary), `agent.session_resume_window` defaults to `86400` (1 day in seconds), `agent.session_idle_timeout` defaults to `900` (15 min; 0 disables idle close), and `agent.env` defaults to `{}` (empty dict)
