...
- Given a config file with no `[agent]` section, when loaded, then `agent.model` defaults to `None` (SDK default), `agent.allowed_tools` defaults to `["Read", "Glob", "Grep"]`, `agent.cli_path` defaults to `None` (SDK bundled binary), and `agent.session_resume_window` defaults to `86400` (1 day in seconds)
