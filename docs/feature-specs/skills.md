...
**Acceptance Criteria**:
- Given eligible skills and a relevant latest message, when the classifier selects a skill, then a single hidden message (`customType: "skill-content"`, not rendered to the user) is injected containing the skill's body (YAML frontmatter stripped, as `/skill` does), wrapped as `<injected-skill name="…">` and preceded by a one-time preface noting no `/skill` load is needed; multiple matches share one message, each in its own wrapped section
