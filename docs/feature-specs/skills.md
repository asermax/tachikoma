...
| R13 | Proactive skill selection feeds the classifier the recent conversation, the latest user message, and the catalog of eligible skills (name + description); it returns skill names validated against the eligible set, then reads and injects each match's full content; the pass is best-effort — a classify failure, a deadline abort, or a skill whose content cannot be read (or is empty) skips that skill and never blocks the response; the classify is bounded by a short, request-cancelling deadline (it aborts the underlying call rather than racing it), and a transient status line surfaces while it runs |
