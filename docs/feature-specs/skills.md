...
| R13 | Proactive skill selection feeds the classifier the recent conversation, the latest user message, and the catalog of eligible skills (name + description); it returns skill names validated against the eligible set, then reads and injects each match's full content; the pass is best-effort — a classify failure, a timeout, or a skill whose content cannot be read (or is empty) skips that skill and never blocks the response; a transient status line surfaces while it runs |
...
- Given the classifier fails or times out, then the injection is skipped and the response is never blocked
