...
| R8 | Per-message pipeline runs registered MessageContextProvider instances on every message (not just first message of session), receiving the `IncomingMessage` envelope and the session's existing context entries |
...
A second pipeline variant runs on every message (not just the first message of a new session). Providers receive the `IncomingMessage` envelope (carrying the message text and typed metadata like pinned skills) and the session's existing context entries, enabling them to determine what's already loaded and avoid redundant work. The pipeline also passes the SDK session ID, enabling providers to fork the session for conversation context. The coordinator passes the `IncomingMessage` envelope directly to the pipeline; the background task executor constructs an `IncomingMessage` from the task's prompt and pinned skills.
