...
| R21 | Typed message envelope: the coordinator's message-buffer / deferred-queue / per-turn inbox carry a `MessageEnvelope` abstract base with property hooks for the per-kind behavior consumers depend on — `sdk_input` (the string yielded to the SDK), `pinned_skills`, `force_new`, `runs_pre_processing`, `runs_boundary_detection`, and `target_session_id`. Concrete subtypes include `TextMessage` (text + pinned_skills + force_new + target_session_id + external_id), `ButtonTapMessage` (value + target_session_id, with `runs_pre_processing=False` so the pre-processing pipelines are skipped for taps), and `ReactionMessage` (added + removed emoji sets + target_session_id + external_id, with `runs_pre_processing=False` and `runs_boundary_detection=False` so reactions skip the pre-processing pipelines and bypass session-boundary detection entirely — both cold-start resume and active-session checks). `target_session_id` is a property hook on the base class (returns `None`) and a dataclass field on each concrete subtype; `external_id` is a dataclass field on `TextMessage` and `ReactionMessage` only (carries the platform-specific message ID; `ButtonTapMessage` does not carry external_id). SDK-input shaping happens at a single rendering site reading `envelope.sdk_input`. Producers (channels, task executor, priority-buffer consumers) construct envelopes at the coordinator boundary. See [DES-013](../../design/DES-013-typed-envelope-with-property-hooks.md) |
| R22 | Session routing via `target_session_id`: when a message envelope carries a non-None `target_session_id`, the coordinator closes the current session (triggering async post-processing) and reopens the named target session before processing the message; this short-circuits boundary detection entirely. The mechanism is generic — any message type can trigger a session switch via `target_session_id` (not limited to reactions). If reopen fails (validation failure, already open), falls back to fresh session creation |
...
- Given a `ReactionMessage` envelope carrying the `added` and `removed` emoji-set diff, when it flows through the coordinator, then `sdk_input` renders canonical prose describing the reaction change, `runs_pre_processing` returns False so both pre-processing pipelines are skipped, and `runs_boundary_detection` returns False so both cold-start resume and active-session boundary detection are skipped (reactions continue whatever session is current — or open a fresh one when no session exists — without invoking the boundary detector)
...
### Session Routing via target_session_id (R22)

When a message envelope carries `target_session_id`, the coordinator short-circuits its normal session management: it closes the current session (triggering async post-processing), reopens the named target session, and processes the message there — skipping boundary detection entirely. If the target session fails resumption validation, a fresh session is created as fallback.

**Acceptance Criteria**:
- Given a message envelope with `target_session_id` set to a session other than the current active session, when the coordinator processes it, then the current session is closed, the target session is reopened, and the message is processed in the target session context
- Given a message envelope with `target_session_id` set to the current active session, when the coordinator processes it, then no session switch occurs and the message processes normally
- Given a message envelope with `target_session_id` set to a session that fails resumption validation, when the coordinator attempts to reopen, then a fresh session is created as fallback with a warning log
- Given a message envelope with `target_session_id` but no active session, when the coordinator processes it, then it attempts to reopen the target session directly; if reopen fails, a fresh session is created
