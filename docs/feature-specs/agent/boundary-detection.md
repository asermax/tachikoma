...
| R23 | `target_session_id` routing: when a message envelope carries a non-None `target_session_id`, the coordinator skips both the cold-start resume branch and the active-session boundary-detection branch, closing the current session and reopening the named target session directly. This is distinct from R21 (`force_new` unconditionally creates a fresh session) and R22 (`runs_boundary_detection=False` continues the current session without invoking the detector) — `target_session_id` explicitly names the target session to reopen. Used by reaction-based session routing (the channel looks up the external ID, determines the target session, and sets `target_session_id` on the envelope) and future reply-based switching mechanisms |
...
- Given a message with `target_session_id` set to a session other than the current active session, when the coordinator processes it, then both the cold-start resume branch and the active-session boundary-detection branch are skipped; the current session is closed and the target session is reopened (R23)
- Given a message with `target_session_id` set to a session that fails resumption validation, when the coordinator attempts to reopen, then a fresh session is created as fallback (R23)
