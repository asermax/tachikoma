...
- Given the git hook, when it runs, then a git repository is initialized in the workspace with a fixed committer identity and an initial empty commit, if no `.git` directory exists; after initialization (or if already initialized), the workspace is synced with the origin remote via smart pull if an origin remote is configured, with sync failures logged and startup continuing normally
- Given the database hook, when it runs, then the shared Database instance is created with async engine and session factory. ORM tables are created idempotently via `create_all()`. A tracked migration system queries applied migrations from the database, records the initial schema as applied (for existing databases this stamps the migration without executing SQL), and executes any pending migrations in order — each in its own transaction. The Database instance is stored in `ctx.extras["database"]`
- Given the plugins hook, when it runs, then the `.tachikoma/plugins/` directory is created, `.tachikoma/plugins/` is appended to workspace `.gitignore` (idempotently), declared plugin sources are reconciled (first-time materialization for new plugins, symlink migration for existing local-source plugins, orphan cleanup), manifests are parsed, the PluginManager is constructed with PluginStateRepository and stored in `ctx.extras["plugin_manager"]`, plugin skill paths are stored in `ctx.extras["plugin_skill_paths"]` for the skills hook to consume, and a daily plugin update check job is registered with the central scheduler (idempotently)
- Given the skills hook, when it runs, then the `skills/` directory is created if it doesn't exist, the SkillRegistry is created with built-in and workspace sources, plugin skill paths from `ctx.extras["plugin_skill_paths"]` are registered as namespaced sources, and the registry is stored in `ctx.extras["skill_registry"]`
