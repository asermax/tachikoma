...
- Given the plugins hook, when it runs, then the `.tachikoma/plugins/` directory is created, `.tachikoma/plugins/` is appended to workspace `.gitignore` (idempotently), declared plugin sources are reconciled (materialized from source, manifests parsed), the PluginManager is constructed and stored in `ctx.extras["plugin_manager"]`, and plugin skill paths are stored in `ctx.extras["plugin_skill_paths"]` for the skills hook to consume
