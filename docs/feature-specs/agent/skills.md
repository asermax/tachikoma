...
| R25 | Skills can optionally contain a `workflows/` folder with multiple workflow definitions, each as a subdirectory of ordered steps |
| R26 | Skill registry discovers workflow definitions alongside skills and exposes them for lookup |
| R27 | Built-in `workflow-authoring-guide` skill ships with the package |

## Behaviors

### Skill Organization (R0, R25)
...
- Given a skill with a `workflows/` subdirectory, when the registry loads the skill, then workflow definitions are discovered and available via `get_workflow(skill_name, workflow_name)` (R26)
...
- Given the built-in directory exists, when the hook runs, then `workflow-authoring-guide` is loaded as a built-in skill (R27)
