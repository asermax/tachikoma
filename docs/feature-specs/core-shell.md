...
- Given a config file that only sets `agent.model`, when loaded, then that value applies and every other value falls back to its default (e.g. `agent.processorModel = "anthropic/claude-haiku-4-5"`, `channels.default = "repl"`)
- Given sections are missing entirely, when loaded, then defaults apply: `workspace.path = "~/tachikoma"`, `agent.model = "anthropic/claude-opus-4-5"`, `agent.thinkingLevel = "medium"`, `agent.searcherModel = "anthropic/claude-opus-4-5"`, `agent.processorModel = agent.classifierModel = "anthropic/claude-haiku-4-5"`, `logging.level = "info"`, `logging.pretty = true`, `channels.default = "repl"`, `sessions.idleCloseSeconds = 900`, `sessions.resumeWindowSeconds = 86400`, `scheduler.timezone` unset, `extensions = {}`
