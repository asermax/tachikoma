...
| R8 | A `preFinalize` post-processor commits each dirty project with a generated descriptive message (deterministic dated fallback) and pushes via `smartPush`, in parallel with per-project error isolation; a rebase conflict during `smartPush` is handed to an agent resolver before falling back to abort (local commits preserved) |
