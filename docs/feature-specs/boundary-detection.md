...
| R19 | Whenever a checkpoint is set — manually (`/checkpoint`) or automatically (the classifier's `set-checkpoint`, or the R15 system-origin side-task checkpoint) — a hidden in-context instruction is injected telling the assistant to handle the side task with full, unhurried focus and no pressure to return to the parked main line, so a tangent gets the same depth of response as a standalone conversation (issue-411). The entry is a `custom_message` (`display:false`, in LLM context) appended at checkpoint-set time, so it rides every tangent turn; it is type `custom_message` (not `message`), so the empty-tangent guard does not count it and the tangent summary does not render it, and it is parked away with the tangent when the checkpoint is summarized |
