...
| R6 | `set-checkpoint` parks the main line: a checkpoint is recorded at the current main-line tip (in the trunk's boomerang-state), the message continues inline as the tangent's first turn, and the decision is surfaced. Only one checkpoint is active at a time; a new one overrides; it persists across restart (it lives on the session file) and is cleared when its tangent is summarized away. It is emitted only when no checkpoint is already active; and only when the current topic is still open — a topic that has reached a natural finish has no main line to park, so an unrelated message then shifts rather than checkpointing (the classifier infers open-vs-finished from the history it reads) |
| R7 | `summarize-to-checkpoint` folds the tangent taken since the active checkpoint into a `branch_summary` rooted at the checkpoint, re-seats the leaf onto the summary (the main line resumes at the checkpoint), clears the checkpoint, and surfaces the decision. It is recognized when the message returns to the main line — either it explicitly references going back to / resuming what was discussed before the side task, its topic clearly matches the main-line topic, or (the common case) the side task looks done and the message does not follow on from it but fits the main line as its next step, even without naming it (the classifier reads the pre-checkpoint conversation from the history) — and emitted only when a checkpoint is active that has at least one conversational turn after it (the empty-tangent guard). A return to the main line is not a new topic, so it is never a `shift` |
...
| R10 | Manual controls mirror the automatic ones. `/checkpoint` sets a checkpoint at the tip (idempotent at the tip — setting the same tip twice is a no-op with a notice); `/back` summarizes the tangent since the checkpoint back into it (no checkpoint → notice; no tangent → notice and no summary created). `/checkpoint` and `/back` accept optional trailing text: it is stripped and streamed as the first turn of the tangent (or the resumed main line), skipping the classifier — the same prefix-strip flow as `/new <text>`; `/rollback` discards any trailing text and replays only the original triggering turn. A bare command marks the message handled and acknowledges immediately; its decision label rides the ack, not a streamed header |
...
| R15 | A message with `metadata.boundary === "skip"` appends with classification skipped — except a system-origin side task (`origin === "system"`, not a `replay`) arriving while a main line is parkable (`hasAssistantTurnSinceBase`) and no checkpoint is active, which parks the main line via an automatic checkpoint and runs as the tangent's first turn (issue-411); all other skips (non-system, replays, an already-parked or non-parkable trunk, or `autoSetCheckpoint = false`) keep the fast-path. A `/new `-prefixed (`forceNew`) message forces a topic shift even when detection is off. A `metadata.forcedBranchId` (a Telegram reply/reaction/button resolving a referenced message to its branch) force-routes: same live branch → append; an earlier recorded branch → forced collapse + related-context injection; an unknown id → fall through to detection |
| R16 | `[extensions.boundary] enabled = false` disables topic-shift detection (the classifier is not run); manual `/checkpoint`, `/back`, `/rollback`, forced `/new`, and forced-branch references still work. Two per-result kill-switches — `autoSetCheckpoint` and `autoSummarizeToCheckpoint` (both default `true`) — disable each automatic checkpoint result independently (the classifier's auto `set-checkpoint`/`summarize-to-checkpoint` and the R15 system-origin side-task checkpoint) without losing the manual commands or topic detection; a suppressed result degrades to `continue` |
...
| R19 | Whenever a checkpoint is set — manually (`/checkpoint`), automatically (the classifier's `set-checkpoint` or the R15 system-origin side-task checkpoint), or by rollback Case B (reversing an auto `shift` to a checkpoint) — a hidden in-context instruction is injected telling the assistant to handle the side task with full, unhurried focus and no pressure to return to the parked main line, so a tangent gets the same depth of response as a standalone conversation (issue-411). The injection goes through one `setCheckpointAndFocus` entry point called from every checkpoint-set site, so it cannot be forgotten at a new one. The entry is a `custom_message` (`display:false`, in LLM context) appended at checkpoint-set time, so it rides every tangent turn; it is type `custom_message` (not `message`), so the empty-tangent guard does not count it and the tangent summary does not render it, and it is parked away with the tangent when the checkpoint is summarized |

## Behaviors

### Topic Classification (R0, R1, R4, R5)

The inbound middleware (`src/extensions/boundary/index.ts`) calls `classifyShift` (`src/extensions/boundary/classifier.ts`), which forks the live branch into a throwaway headless session on the `classifier` tier, asks it which decision fits the incoming message, and returns one of `continue` / `shift` / `set-checkpoint` / `summarize-to-checkpoint`. The middleware injects whether a checkpoint is active so the classifier only offers the checkpoint decision valid for the current state. The `set-checkpoint` result targets a self-contained side task **distinct from** the current thread that the user interleaves while mid-workflow or mid-conversation — a note or reminder capture, logging something, a quick lookup, or a background task or daily ceremony starting its own short exchange (the classifier reads that context from the conversation history): the main line is parked and resumes once the side task is done, rather than shifting to a fresh topic. The discriminator from `continue` is that the message clearly starts a separate task rather than following on from the current one (regardless of how many turns the side task may take); the discriminator from `shift` has two axes — substantiveness (a minor interleaved side task to fold away vs. a substantive new main topic), and whether the current topic is still open. `set-checkpoint` presupposes an open main line to park and resume, so it is gated on an open topic: once the current topic has reached a natural finish (the last exchange wrapped it up), an unrelated message shifts (even a small capture like an expense or a reminder), since there is no main line left to park. The classifier infers whether the current topic is open or finished from the conversation history the shadow fork already reads, so — unlike `checkpointActive` — no finish-state signal is injected. The `summarize-to-checkpoint` result fires when the message returns to the main line — either it explicitly references going back to / resuming what was discussed before the side task, its topic clearly matches the main-line topic, or (the common case) the side task looks done and the message does not follow on from it but fits the main line as its next step, even without naming it (read from the pre-checkpoint conversation the shadow fork already sees); a return to the main line is not a new topic, so it is never a `shift`. `shift` is reserved for a substantive new topic, and ambiguity conservatively falls back to `continue` (or `shift`).
...
### Checkpoints (R6, R8, R11, R19)

A checkpoint is the main-line tip marker that lets a tangent run inline and later be summarized back.

**Acceptance Criteria**:
- Given an active trunk at a main-line tip, when the user sends `/checkpoint`, then the current tip is recorded as the active checkpoint (overriding any prior), a tangent-focus instruction is injected (R19), and a confirmation is surfaced
- Given an active trunk at a main-line tip, when the user sends `/checkpoint <text>`, then the checkpoint is set (or already active, idempotently) and `<text>` is streamed as the tangent's first turn with classification skipped (the prefix-strip flow `/new <text>` uses); a bare `/checkpoint` acks without starting a turn
- Given `/checkpoint <text>` with no conversation to checkpoint, when it runs, then the warning notice is surfaced and `<text>` is not streamed
- Given a checkpoint is set by any path (manual `/checkpoint`, automatic `set-checkpoint`, or the R15 system-origin side-task checkpoint), when the focus instruction is injected, then it is a hidden `custom_message` that rides the tangent without counting as a tangent turn or appearing in the tangent summary
...
- Given an active checkpoint with a tangent, when the user sends `/back <text>`, then the tangent is summarized and `<text>` is streamed as the resumed main line's first turn with classification skipped; a bare `/back` acks without starting a turn
- Given `/back <text>` with no active checkpoint or no tangent, when it runs, then the notice is surfaced and `<text>` is not streamed
...
- Given `/rollback <text>`, when it runs, then the trailing text is discarded — only the original triggering message is replayed under the corrected framing (unlike `/checkpoint`/`/back`, `/rollback` takes no argument)
...
The middleware checks metadata before classification. Order: a null trunk and the `boundary === "skip"` side-task rule, then the manual commands, then `forcedBranchId`, then `forceNew`, then (when enabled) classification.

**Acceptance Criteria**:
- Given a system-origin message with `metadata.boundary === "skip"` (`origin === "system"`, not a `replay`) arriving while a main line is parkable and no checkpoint is active, when the middleware runs, then it sets a checkpoint, injects the tangent-focus instruction (R19), surfaces an informational (non-rollbackable) header, and calls `next()` — the message streams as the tangent's first turn, with no classification
- Given any other `metadata.boundary === "skip"` message (non-system, a replay, an already-parked or non-parkable trunk, or `autoSetCheckpoint = false`), when the middleware runs, then it calls `next()` immediately — no command, collapse, or classification
