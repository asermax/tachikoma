...
| R2 | A `memories` context section (registered via `app.agent.use(persistentContextSection("memories", { provide }), { sessionScopes: ["main", "background"] })`, built by `buildMemoryContext`) is injected once per session in both main and background: a layout section describing the stores (including the read-only/post-processing note) plus the parsed facts and preferences indexes; it yields empty content (no injection) when `memories/` does not exist |
