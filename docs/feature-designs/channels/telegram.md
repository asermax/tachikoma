...
**Then**: `_extract_reply_target` extracts the replied-to message ID. `_resolve_reply_target` looks up the session, finds it differs from the active session. `target_session_id` is set on the `TextMessage` envelope. The envelope is deferred via `enqueue_deferred()` (per DES-009 — session switches always use the deferred queue so the coordinator handles pre-validation). When the deferred queue drains, the coordinator's `_route_to_target_session` pre-validates the target via `can_reopen_session`, closes the current session (triggering async post-processing), reopens the target, and processes the message in the resumed session.
**Rationale**: Primary use case — explicit, precise session resumption via reply-to gesture.
...
**When**: The handler processes the message
**Then**: `target_session_id` is set on the envelope. The envelope is deferred via `enqueue_deferred()` with `target_session_id`. When the current turn completes and the deferred queue drains, the coordinator performs the session switch (pre-validation + close + reopen).
**Rationale**: Session switches happen between turns, never mid-response (per DES-009).

### Scenario: Reply to message from current session

**Given**: The user replies to a message from the active session
**When**: The handler processes the message
**Then**: `_resolve_reply_target` returns `None` (target session matches active). No `target_session_id` is set. Message routes normally.
...
**Given**: The user replies to a message whose external ID is not recorded
**When**: The handler processes the message
**Then**: `_resolve_reply_target` returns `None`. No `target_session_id` is set. Message routes through normal boundary detection.
**Rationale**: Graceful degradation — feature silently degrades to current behavior for unmapped messages.

### Scenario: Reply targets a stale session

**Given**: The user replies to a message from a session that fails validation (transcript missing, too old)
**When**: The coordinator's `_route_to_target_session` calls `can_reopen_session`
**Then**: `can_reopen_session` returns `False`. `_route_to_target_session` returns `(active, is_new_session, routed_successfully=False)`. The coordinator checks `if not routed:` and yields a `Status` event with message "Could not resume that conversation — its context is no longer available." The current session is preserved. The message falls through to normal processing.
**Rationale**: The user gets a clear explanation instead of an unexpected new session. Pre-validation prevents unnecessary session churn.

### Scenario: Reply combined with /new command

**Given**: The user sends a reply that also contains the `/new` command
**When**: The handler processes the message
**Then**: Both `_detect_command` and `_extract_reply_target` return values. The reply-to gesture identifies the target session: `target_session_id` is set and `/new`'s `force_new` is suppressed (set to `False`). The message routes to the target session.
**Rationale**: The reply-to gesture is a more deliberate intent signal than a text prefix.

### Scenario: Media message with reply-to

**Given**: The user sends a photo that is a reply to a past message
**When**: The media handler processes the message
**Then**: `_extract_reply_target` extracts the replied-to message ID. The same session lookup and routing logic applies as for text messages. Busy/idle deferral follows the same pattern (per DES-009).
**Rationale**: Reply-to works for all message types.

### Scenario: Reaction handler reuses _resolve_reply_target

**Given**: The reaction handler needs to look up the session for a reacted-to message
**When**: `_handle_reaction` processes the event
**Then**: The inline session lookup code has been replaced with a call to `_resolve_reply_target(str(event.message_id))`. The same helper now serves both reply-to and reaction routing, avoiding duplication.
**Rationale**: DRY — the session lookup logic (find by external ID, compare to active) is identical for reactions and reply-to. Single helper, single test surface.
