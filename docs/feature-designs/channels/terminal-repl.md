...
|-----------------|----------------|---------------|
| `Repl(Channel)` | Implements Channel protocol with inherited defaults (no tools, no skills). Constructor takes `history_path` and optional `bus` — no coordinator (received in `run()`). Event bus subscription (`BufferedDelivery`) deferred to `run()` to ensure coordinator is set before handlers fire. On `BufferedDelivery`, the REPL wraps `event.prompt` in a `TextMessage` envelope and routes it through the coordinator as a new turn, then fires the per-item `on_delivered` callbacks. Typed user input is similarly wrapped in `TextMessage` at the coordinator boundary per [DES-013](../../design/DES-013-typed-envelope-with-property-hooks.md). Input loop, control flow, exit conditions, interrupt handling. | Owns `PromptSession` with `multiline=True`, custom `KeyBindings` (Enter submits via `validate_and_handle()`, Escape+Enter inserts newline), `prompt_continuation="  "`, `FileHistory` (at `/tmp/tachikoma_repl_history`), and empty-input `Validator`; `__coordinator` private field + `_coordinator` property (same pattern as TelegramChannel); only `BufferedDelivery` is observed — `Notification` and session-task events are handled by the priority buffer (see [delivery/priority-buffer](../delivery/priority-buffer.md)) |
...
- The REPL subscribes only to `BufferedDelivery`; session tasks and notifications are enqueued into the priority buffer by their producers, and the buffer emits one delivery event per ready item (or a single digest event during shutdown flush). The handler routes `event.prompt` through the coordinator and fires each item's `on_delivered` on success.
