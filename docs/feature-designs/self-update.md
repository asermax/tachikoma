...
`index.ts` reads the running version (`readInstalledVersion`), constructs the three seams and the typed KV state wrapper, then wires three entry points: a cron job running `runCheck`, an `app.bootstrap` hook running `reconcileStartup`, and the `upgrade_self` tool backed by `runUpgrade`. Each entry point is a thin orchestrator that calls a pure decision function (`decisions.ts`) and then performs the seam operations the decision dictates.

The decision functions are total, synchronous, side-effect-free transformations over plain data: `decideCheck` (notify / up-to-date / already-notified / loop-guarded / registry-unavailable), `decideUpgrade` (proceed / up-to-date / blocked-failed / registry-unavailable), and `decideStartup` (none / upgradeSucceeded / rollback). Version ordering is its own pure module (`version.ts`) with an `isNewerVersion` that ignores prereleases and unparseable inputs.
...
|-----------|----------------|---------------|
| `src/extensions/self-update/index.ts` | Wiring: read version, build seams + state, register cron job, bootstrap hook, and `upgrade_self` tool | `enabled: false` short-circuits setup entirely; the upgrade tool receives a `() => UpgradeDeps` thunk so deps are assembled once and shared |
...
| `src/extensions/self-update/upgrade.ts` | `runUpgrade`: gate → marker → install → restart | Marker written before install; install failure clears marker + records loop guard; restart does not return on success |
...
| `src/extensions/self-update/tools.ts` | `upgrade_self` agent tool | Returns text only on a non-restart outcome; prompt guidance warns the agent there is no result on success |
...
- **`Restarter.restart(): never`** — the real `ProcessRestarter` re-execs by `spawnSync(process.execPath, process.argv.slice(1), { stdio: "inherit" })` and then `process.exit` with the child's code (Node has no `execv`; running the child synchronously in-line keeps a single live process and inherits the terminal).
