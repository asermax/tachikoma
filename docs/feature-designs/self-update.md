...
The decision functions are total, synchronous, side-effect-free transformations over plain data: `decideCheck` (notify / up-to-date / already-notified / loop-guarded / registry-unavailable), `decideUpgrade` (proceed / up-to-date / blocked-failed / registry-unavailable), and `decideStartup` (none / upgradeSucceeded / rollback / restartCompleted). `runUpgrade` adds one further gate on top of `decideUpgrade`: if `DevInstallDetector.isDevInstall()` reports the running copy is a linked/source checkout, it returns a `dev-install` status and refuses, since installing a published version over a dev checkout would clobber the working tree. Version ordering is its own pure module (`version.ts`) with an `isNewerVersion` that ignores prereleases and unparseable inputs.

State is four concerns in one KV namespace: `lastCheck` (checker bookkeeping + last-notified version), `upgradeMarker` (the in-progress marker bridging an upgrade restart), `restartMarker` (the marker bridging a plain `restart_self` to its next-boot "back online" announcement), and `failedVersion` (the loop guard). The upgrade marker is the linchpin of rollback: `runUpgrade` writes it *before* installing; `reconcileStartup` reads it on the next boot and compares the marker's `targetVersion` to the version now running. Equal means the upgrade landed (announce, clear marker + guard); unequal means the new version never booted (record the target as failed, reinstall `previousVersion`, clear the marker, re-exec back). The marker is always cleared before any restart, so a rollback that itself fails to boot is seen as a clean boot by the next process rather than looping. The `restartMarker` reuses the same bridge for a plain (same-version) restart: `restart_self` writes it before the deferred re-exec, and the next boot announces "back online" and clears it — there is no version comparison or rollback, because a restart carries no version delta. If both markers are present (e.g. a `restart_self` followed an `upgrade_self` before the deferred re-exec), the upgrade marker wins so its rollback still runs.
...
|-----------|----------------|---------------|
| `src/extensions/self-update/index.ts` | Wiring: read version, build seams + state, register cron job, bootstrap hook, `upgrade_self` tool, and `restart_self` tool | `enabled: false` short-circuits setup entirely; the upgrade tool receives a `() => UpgradeDeps` thunk so deps are assembled once and shared; both tools schedule a deferred restart via `app.requestRestart` (handing the `Restarter` thunk to the coordinator) |
...
| `src/extensions/self-update/decisions.ts` | Pure decisions: `decideCheck`, `nextLastCheck`, `decideUpgrade`, `decideStartup` | All total functions over plain data; outcome/gate/action const maps with derived types; loop guard wins over notification and upgrade; upgrade marker wins over a restart marker |
| `src/extensions/self-update/state.ts` | Typed wrapper over the KV namespace: lastCheck, upgrade marker, restart marker, failed version | Thin accessors only — no logic; four concerns share one namespace |
...
| `src/extensions/self-update/upgrade.ts` | `runUpgrade`: gate → dev-install check → marker → install → return `started` | Marker written before install; a dev install short-circuits with a `dev-install` status (no marker/install/restart); install failure clears marker + records loop guard; on success returns a `started` outcome and leaves the restart to the caller (the tool schedules it as deferred) |
| `src/extensions/self-update/startup.ts` | `reconcileStartup`: `decideStartup` → announce (upgrade or restart) / rollback | A restart marker announces "back online" and clears; the upgrade marker is cleared before any restart; rollback records the target as failed first; rollback-install failure emits an urgent notice and stays put |
| `src/extensions/self-update/tools.ts` | `upgrade_self` and `restart_self` agent tools | Both return a result and schedule a deferred restart via `app.requestRestart` (handing the `Restarter` thunk to the coordinator); the restart runs only after the current exchange completes, so the agent always sees a tool result. `restart_self` re-execs in place via the `Restarter` seam without upgrading (for config/state changes) and writes a restart marker before the deferred re-exec, so the next boot announces "back online" |
...
- **`Restarter.restart(): never`** — the real `ProcessRestarter` re-execs by `spawnSync(process.execPath, process.argv.slice(1), { stdio: "inherit" })` and then `process.exit` with the child's code (Node has no `execv`; running the child synchronously in-line keeps a single live process and inherits the terminal). Backs both `upgrade_self` (after install) and `restart_self` (in place). The tools never call it directly: they hand a `() => restarter.restart()` thunk to `app.requestRestart`, and the coordinator re-execs only after the current exchange completes and its graceful drain runs. (The boot-time rollback in `reconcileStartup` is the one direct caller — it runs before any exchange, so there is nothing to defer.) Because the old process stays alive inside `spawnSync` while the replacement boots, `runApp` releases the workspace's instance lock before invoking the deferred restarter — the replacement would otherwise see its own parent as a live foreign holder. The boot-time rollback in `reconcileStartup` restarts directly and does not release first, so its replacement fails the lock check once and boots cleanly on the next supervisor restart — a rare, accepted cost.
...
**Choice**: Persist an upgrade marker (`previousVersion`, `targetVersion`, `startedAt`) in the extension's namespaced KV before installing, and reconcile it on the next boot by comparing `targetVersion` to the running version. Rollback lives in `reconcileStartup`, not in `runUpgrade`. A plain `restart_self` reuses the same KV-marker bridge with a lighter `restartMarker` (`startedAt` only): `restart_self` writes it before the deferred re-exec, and the next boot announces "back online" and clears it — no version comparison, since a restart is same-version. The upgrade marker takes precedence when both are present, so rollback is never skipped.
...
- Pro: A plain restart gets the same "back online" confirmation as an upgrade, reusing one marker mechanism instead of a second delivery path
...
### Deferred restart reuses the graceful drain

**Choice**: The tools do not call `Restarter.restart()` from inside `execute()`. Instead each tool hands a `() => restarter.restart()` thunk to `app.requestRestart`, which stashes it on the coordinator as a pending-restart flag. After the current exchange completes, the coordinator breaks its run loop, its existing graceful-drain `finally` runs (shutdown hooks → drain held deliveries → status), and `app.ts` invokes the thunk after `channel.stop()` / `scheduler.stopAll()` (with the instance lock released immediately before the re-exec) — so the re-exec is the last thing the process does.
**Why**: Re-execing from inside the tool (`spawnSync` + `process.exit`) killed the exchange mid-stream: the agent's response was cut short and exchange processors, held-delivery drain, and compaction never ran. A restart is exactly a graceful drain plus a re-exec, so reusing the coordinator's drain avoids a second teardown path and guarantees the full exchange lifecycle runs first. The coordinator owns the flag (it owns the exchange loop and the drain); `app.ts` owns the re-exec (only the top level can cleanly `process.exit` after channel/scheduler teardown). The post-exchange `if (pendingRestart) break` runs before the run loop's abort check, so a pending restart always wins over a concurrent shutdown signal.
**Alternatives Considered**: Re-execing directly from the tool (the old behaviour — cuts the response short); aborting the run via the `ShutdownController` from inside the tool (works, but conflates a user-requested restart with crash/signal semantics and adds a force-exit timer that has no place in an intentional restart).
**Consequences**:
- Pro: The agent's full response is delivered and post-exchange work runs before the process is replaced
- Pro: No duplicated teardown — the restart rides the same graceful-drain path as a normal shutdown
- Con: The window between a successful install and the re-exec is wider (the exchange must finish first); a crash in that window leaves the upgrade marker for the next boot to reconcile, which rolls back safely but may record an unnecessary loop guard. This is the same recovery as today, just over a slightly longer window
- Con: Unprocessed inbox messages arriving after the restart request are dropped (the loop exits); held deliveries are still drained, and the trunk survives, so this is acceptable for an explicit restart
