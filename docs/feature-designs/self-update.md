...
**Choice**: The tools do not call `Restarter.restart()` from inside `execute()`. Instead each tool hands a `() => restarter.restart()` thunk to `app.requestRestart`, which stashes it on the coordinator as a pending-restart flag. After the current exchange completes, the coordinator breaks its run loop, its existing graceful-drain `finally` runs (shutdown hooks → drain held deliveries → status), and `app.ts` invokes the thunk after `channel.stop()` / `scheduler.stopAll()` — so the re-exec is the last thing the process does.
