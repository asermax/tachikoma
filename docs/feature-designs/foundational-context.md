...
| workflows — step-directory processes | Inline + `references/workflows.md` | main | What/when plus the compact driving model (top-level id, instruction delivery, per-type mechanics, recovery entry) inline; full driving detail and the early-end mechanic demoted (issue-474) |
