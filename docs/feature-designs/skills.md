...
### Delegate skill discovery to pi, and augment its relevance with a conversation-aware classifier

**Choice**: Contribute the skills directory through `resources_discover` and let pi own discovery and loading; on top of that, a per-turn conversation-aware classifier (`suggest.ts`) recommends the skills it judges relevant by injecting a hidden message that tells the agent to load them (`/skill:<name>`). Discovery and loading stay pi-native; relevance is augmented, not replaced — descriptions remain in the system prompt and the agent still loads the skill content itself.
**Why**: pi implements the Agent Skills standard natively with progressive disclosure, so owning a parallel discovery/loading pipeline is redundant. But progressive disclosure leaves the *decision* to load to the model, which (per the SDK's own note) "does not always do this" — so a genuinely relevant skill can be skipped. A conversation-aware recommendation closes that gap with a forceful, in-context nudge while keeping the context lean (no skill bodies pre-injected), at the cost of a per-turn classify call and a load round-trip the agent performs itself.
...
- **Pre-injecting full skill bodies** instead of recommending: considered and not chosen — it spends the tokens of every matched skill's content up front and bypasses progressive disclosure's laziness; recommending keeps the context lean and lets the agent load only what it actually uses.
...
**Consequences**:
- Pro: relevant skills get loaded even when the model would not pick them; relevance uses conversation context, not just description quality
- Pro: context stays lean — only a short recommendation is injected, never skill bodies; discovery and loading stay pi-native; the classifier is a thin, fakeable side-call reusing `SideRunner.classify`
- Con: a per-turn `classifier`-tier round-trip on the hot path when eligible skills exist (plus the agent's own load round-trip, and reliance on it honoring the recommendation — stronger than passive disclosure but not guaranteed) — bounded by the no-eligible-skip, a timeout, a recent-context window, and the per-session `injected` Set (cost decays as a session's skills get recommended), and disableable via `proactiveLoading`
...
**Then**: The handler filters eligible skills, surfaces a transient "Checking for relevant skills…" status, hands the recent conversation + latest message + skill catalog to `classify`, and on a `pdf-tools` match injects one hidden `skill-recommendation` message (`- /skill:pdf-tools — Work with PDFs`) telling the agent to load it before responding. A later turn in the same session does not recommend `pdf-tools` again; an unrelated message recommends nothing.

### Scenario: Proactive recommendation suppressed inside post-processing
...
**Then**: `app.agent.isForking()` is true, so the handler returns immediately — no classify call, no recommendation — leaving proactive loading to genuine top-level turns only.
...
- Proactive recommendation is best-effort and silent: the injected `skill-recommendation` message is `display:false` (the adapter surfaces only assistant text/thinking/tool events, so nothing reaches the channels), and the only visible surface is the transient status line. The per-session `injected` Set lives in the factory-invocation closure, so a new/resumed session re-evaluates from scratch (a benign re-recommendation at worst). See [agent-integration](agent-integration.md) for `isForking()`.
- Tests: `tests/skills/agents.test.ts` (discovery against tmp-dir fixtures), `tests/skills/builtins.test.ts` (built-in agent shape), `tests/skills/delegate.test.ts` (tool behavior against a faked runner, including the built-in and isolation), `tests/skills/index.test.ts` (unconditional registration, the background-task opt-in, and the `proactiveLoading` gate/default), `tests/skills/suggest.test.ts` (proactive recommendation against a faked classifier: eligibility, isForking skip, timeout, multi-skill packing, dedup, status, classify payload), `tests/skills/reload.test.ts` (the `/reload` command and the `reload_resources` tool registered by `reload.ts`), `tests/agent/manager.test.ts` (`isForking()` across nested/parallel forks), `tests/agent/prompts.test.ts` (role prompt composition)
