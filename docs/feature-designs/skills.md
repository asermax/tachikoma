...
- Note: two producers of hidden `skill-content` messages exist — this classifier (conversation-picked, per-turn) and the agent layer's force-load factory (`src/agent/force-load-skills.ts`, see [agent-integration](agent-integration.md)): caller-directed (an `open()`/`run()` option naming specific skills), once per session per skill, for grounded headless runs (skill-evolution's proposal agent force-loads the two bundled authoring guides)
