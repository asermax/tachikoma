...
| `src/extensions/telegram/store.ts` | `TelegramMessageStore implements ChannelMessageStore` over `app.db`: `record()` upserts a message id↔session row, `findSessionId()` resolves a replied-to message back to its owning session id | Owns the `channel_messages` reads/writes that previously lived on the core `SessionRegistry`, removing the core→telegram dependency; `findSessionId` reads the session id straight off the mapping row (the only thing reply routing needs) rather than round-tripping the registry |
