...
- Con: Reactions and replies only route when the referenced message was recorded; an unrecorded reaction is dropped with a notice, while an unrecorded text/media reply proceeds with an unresolved-reply hint (messages predating this feature, or sent outside `respond()`, are not resolvable)
...
**Then**: `mapReaction` diffs the emoji sets and submits a turn reading `The user reacted 👍 to a previous message.` with `metadata.reaction` and `metadata.replyToMessageId`; if that message id maps to another session, `routeReply` adds `resumeSessionId` so the reaction lands in the owning conversation. Unless the reaction targets the session's *most recent* message (already at the bottom of the conversation), the reacted-to message's text is recovered from its session-tree entry and prepended as a truncated `Reacted to:` quote so the agent knows which message the emoji targets; a reaction to the session's *latest* message is left bare. A reaction from any other user id is dropped, and a no-change reaction update submits nothing. If the reacted-to message id is not recorded, the handler sends a notice that the conversation couldn't be found and drops the reaction rather than steering an unrelated session.
