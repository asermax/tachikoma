...
**Choice**: `StreamRenderer` carries a `header` field, set via `setHeader()` before streaming begins, and `compose()`/`finalize()` prepend it as `header\n\nbody` (the header rendered as a bold `**label** — note`) on every edit so the streamed text never overwrites it. The header is dropped after the exchange (turn-scoped — never carried to a later turn). It is best-effort: when the composition exceeds the 4096-char edit limit the transient live line is dropped first, then — if still too long — the header itself is dropped (and logged at `info`) for the rest of the exchange; on a render failure the descriptor is logged and the response streams without it. This is the canonical instance of [DES-009](../design/DES-009-turn-scoped-anchored-header.md).
