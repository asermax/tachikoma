...
**Choice**: The channel records every handled message id ↔ session in `channel_messages` (inbound and outbound, written in `respond()` once routing has settled). When an inbound message replies to — or a reaction targets — a recorded id, `routeReply` resolves the owning session and stamps `metadata.resumeSessionId` on the turn. The boundary middleware checks for `resumeSessionId` first and, when it points to a different known session, resumes it and returns *before* running topic classification. An unknown target — one not in the store — is handled by kind: `routeReply` leaves a reaction without a `resumeSessionId`, and the reaction handler detects that absence and notifies the user before dropping it (a reaction's meaning is bound to the referenced message, so submitting it to an unrelated session would mis-attribute intent); a text/media reply instead proceeds under normal routing with a hint appended to the turn, so the agent knows the reply couldn't be matched to its original conversation.
