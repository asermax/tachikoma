...
- The `Registrations` object filled here is consumed by the coordinator (context providers, middleware, processors, channels) and the `AgentManager` (pi factories, system prompt builders) — both documented in their own areas
