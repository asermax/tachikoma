...
| `src/extensions/api.ts` | `AppContext`, pipeline contracts (`ExchangeProcessor`, `PostProcessor`, `InboundMiddleware`, `ContextSectionRegistration`), `defineExtension` | `defineExtension` is identity — purely a typing aid for the `C` config parameter |
