...
`classifyShift` (`src/extensions/boundary/classifier.ts`) forks the live branch into a throwaway session on the `classifier` tier and asks it which decision fits, returning a free-form JSON object parsed tolerantly. It is checkpoint-aware: the middleware injects whether a checkpoint is active, and the prompt only describes the checkpoint decision valid for the current state. The `set-checkpoint` criterion is a short, **unrelated**, self-contained side request (e.g. an expense logged mid-workflow); the `summarize-to-checkpoint` criterion is a clear return to the main line — by explicit "going back" phrasing or by the message's topic matching the pre-checkpoint conversation (read from the history the shadow fork already sees), not only the side task being finished; `shift` is reserved for a substantive new main topic, so the two 'unrelated' decisions stay distinct and a return to the main line is never a `shift`. Anything unrecognized fails open to `continue`.
