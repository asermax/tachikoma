...
| `src/extensions/boundary/commands.ts` | `/checkpoint`, `/back` manual handlers | Detected before the classifier, mark the message handled, and ack immediately — no agent turn, so the decision label is in the ack, not a streamed header |
| `src/extensions/boundary/rollback.ts` | `/rollback`: eligibility + immediacy, rewind, opposite transition, `reversed` marker, replay | Immediacy = exactly one user-message turn after the decision marker (counting user messages, not entries, so an appended summary isn't read as a new turn); `handled` set unconditionally so the command text never reaches the agent |
