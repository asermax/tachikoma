...
**Then**: `detectBoundary` confirms the id is among the offered candidates, the middleware calls `resumeSession`, and the coordinator — after verifying the target is not quarantined (errored) and its `piSessionFile` exists on disk (skipping the resume and keeping the active session if it is errored, null, or missing) — reopens the record and its pi session file, then injects a `bridging-context` block summarizing any sessions that closed while the resumed one was dormant (`listClosedBetween`).
