...
- Executor ↔ Task tools: the task-tools MCP server is injected via `extra_mcp_servers` (wired in `__main__.py` alongside `git-tools`) and merged into SDK options without shadowing per-invocation servers — enabling agents to call `create_task`, `list_tasks`, `get_task`, `update_task`, and `delete_task` during execution
