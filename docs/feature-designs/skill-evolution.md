...
| `src/extensions/skill-evolution/report.ts` | Reporter: build prompt/goal, emit the dispatch event | Default prompt is notification-only; run summary inline with one review-ready block per verified proposal — git facts paired with the reported reasoning (first report per branch), sanitized so agent-authored markdown cannot restructure the section |
...
report_proposals(proposals: [{branch, skill, pattern, description, problem,
    rootCause, evidence}])  — required terminal call; captured in a closure
    variable the host reads after the run; the three reasoning fields restate
    the acted-on pattern page (Problem/Root cause/Evidence) tailored to the
    authored change, and a blank-or-whitespace field refuses the call
    (trim-then-test, captures nothing)
```
...
Pattern page 1—1 Skill (loose: a pattern names the skill it indicts — or, when no skill covers the work, the workflow that needs one)
  ...
  └─ runPhasedPostProcessors ── main phase (allSettled, ∥ memory-trunk-close)
       └─ skill-evolution-trunk-close.process({trunk, log, status})
            ...
            └─ verified ≥ 1? → report: emit DISPATCH_BACKGROUND_TASK_EVENT
                   {prompt: postWorkPrompt ?? default + run context (reasoning
                   blocks, one per verified proposal), goal: run summary, source}
...
**Choice**: `src/events.ts` carries `DISPATCH_BACKGROUND_TASK_EVENT` and its payload; skill-evolution emits it; the tasks extension subscribes (`src/extensions/tasks/dispatch.ts`) and creates the ad-hoc pending background instance. The prompt is `postWorkPrompt ?? default notification-only prompt` (forwarded verbatim), seeded with the run summary (proposals, patterns, log path) plus one review-ready block per verified proposal — the row's git facts paired with the entry's reported reasoning, matched by branch, rendered so no agent-authored line can restructure the section — and an explicit `goal` (which skips the background runner's goal-extraction call). The reasoning is agent-reported context that rides the dispatch only ([DES-010](../design/DES-010-agent-driven-git-host-verified.md)): it never reaches the impact ledger and never feeds a verification decision, and the framing over the blocks is informational (what a pull-request body should carry), never an instruction — so the same rendering stays coherent under the notification-only default and under a configured acting prompt.
