...
- Pre-processing pipeline runs registered context providers on first message of new session (see [pipeline design](pre-processing-pipeline.md)); memory indexes are injected as foundational context at startup (see [memory context retrieval](../memory/memory-context-retrieval.md))
...
11. Creates PreProcessingPipeline, registers ProjectsContextProvider(workspace_path=workspace_path) and SkillsContextProvider(agent_defaults, registry=skill_registry)
