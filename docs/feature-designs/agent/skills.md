...
SkillsContextProvider.provide(message: IncomingMessage, existing_entries=entries)
    │
...
- SkillsContextProvider ↔ Per-Message Pipeline: registers via `msg_pre_pipeline.register(provider)`; `provide(message: IncomingMessage, existing_entries=entries)` called on every message
...
- Per-Message Pipeline ↔ Coordinator: `pipeline.run(msg: IncomingMessage, existing_entries=entries)` returns `list[ContextResult]`; coordinator passes the `IncomingMessage` envelope directly; saves new entries and derives agents from all entries + registry
...
└── provide(message: IncomingMessage, *, existing_entries) → list[ContextResult] | None
    ...
    ├── Classify via query() (DES-007) using message.text
