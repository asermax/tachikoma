...
- The `push` MCP tool's `scrub_paths` argument is parsed by a `field_validator(mode="before")` on `PushArgs` that defensively decodes a JSON-encoded string variant produced by an SDK transport bug. The schema continues to advertise `array` only — see [DES-006](../../design/DES-006-sdk-mcp-tool-server-factory.md) for the reusable pattern.
