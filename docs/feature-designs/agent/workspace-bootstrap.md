...
- The plugins hook appends `.tachikoma/plugins/` to the workspace `.gitignore` idempotently (the install directory is regenerable from config). It also constructs the `PluginManager` and stores it in `ctx.extras["plugin_manager"]` for `__main__.py` to create the plugin MCP tools server after bootstrap completes.
