...
- Hook registration order in `__main__.py`: workspace → logging → git → database → plugins → skills → context → memory → sessions → tasks → media → telegram. Logging runs before git so that loguru's file handler is configured before any hooks emit log messages. Git hook initializes the repo and syncs workspace from remote. Database creates the shared Database with async engine, runs `create_all()` for ORM-defined tables, and delegates to the tracked migration runner for versioned schema changes. Migrations are forward-only, each running in its own transaction against a `schema_migrations` tracking table. Plugins runs after git (needs the repo initialized for git-based sources) and before skills (populates `ctx.extras["plugin_skill_paths"]` for the skills hook to consume). Skills runs before context so that the skills directory and registry are created before context initialization. Media runs after tasks and before telegram — the temp folder must exist before polling starts. Telegram runs last so all other subsystems are initialized before channel-specific validation.
- The plugins hook appends `.tachikoma/plugins/` to the workspace `.gitignore` idempotently (the install directory is regenerable from config). It performs first-time-only materialization for new plugins (not re-materialization on every startup), migrates existing local-source copies to symlinks, creates a `PluginStateRepository` from the database session factory, constructs the `PluginManager` with the state repo, and stores both in `ctx.extras`. A daily plugin update check job is registered with the central scheduler (DES-010) via `__main__.py` after bootstrap completes.
