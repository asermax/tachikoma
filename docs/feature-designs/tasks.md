...
- Notifications extension: background-task notices (failures, the stuck/expired sweeps, the `ask_user` pause) emit a `NotifyPayload` on the shared `notify` app event, so they flow through the notifications router's batching, dedup, severity→tier mapping, and idle gating like every other producer (see [notifications.md](notifications.md)). The tasks extension imports the notify contract (`NOTIFY_EVENT`/`NotifyPayload`/`SEVERITIES`) from neutral `src/events.ts` (re-exported by the notifications extension) and carries no notice event of its own
- Agent manager: `app.agent.side.openBackgroundSession` (a persistent pi session bound with the background factories + custom tools, prompted repeatedly across the loop) and `app.agent.side.run` (a tool-enabled headless run with read-only filesystem tools — derives the run's goal at run start, reading any files the prompt references) power background execution
...
| `src/extensions/tasks/dispatch.ts` | Subscribes to `task:dispatch-background` (`DISPATCH_BACKGROUND_TASK_EVENT` from neutral `src/events.ts`): validate payload → `repository.createAdHocInstance` (pending background, `definitionId: null`) | Programmatic dispatch channel for extensions; a blank/missing `goal` normalizes to `null`, a prompt-less payload warns and drops (never thrown into the emitter); the existing 60 s tick dispatches the instance like any ad-hoc `run_task_now` |
...
- Con: `BackgroundSide` is `openBackgroundSession`/`run` (the run-start extraction uses `run`, not `classify`); tests mock a fake persistent session plus a fake `run`
...
**Why**: A failure or pause happens when no agent turn is producing output, so it must be raised programmatically; routing it through `notify` rather than a bespoke `deliver` gives background notices the router's batching, dedup (re-emit/retry storms collapse), severity→tier mapping, and idle gating for free, and keeps tasks from re-implementing notice formatting. A success has a live agent that can decide whether the result is worth surfacing, so the executor stays silent there. The notify contract lives in neutral `src/events.ts` (the notifications extension re-exports it), so cross-extension emitters import it without touching another extension's directory.
**Consequences**:
...
- `DISPATCH_BACKGROUND_TASK_EVENT` (neutral `src/events.ts`, subscribed in `dispatch.ts`) is the programmatic dispatch channel — the [skill-evolution](skill-evolution.md) reporter is the emitter, requesting an ad-hoc background run per nightly proposal batch (R29)
