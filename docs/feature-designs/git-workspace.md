...
The extension (`src/extensions/git/index.ts`) wires a bootstrap hook, two pi extension factories (a tools factory and a bash guardrail factory), and a finalize-phase post-processor. The reusable git *primitives* live in a neutral **core module** `src/git/` — `git.ts` (promisified `execFile` subprocess primitives that never shell out), `commit.ts` (stage-all + message generation), and `sync.ts` (divergence classification and the push/pull state machines, including the `RebaseResolver` type), and `resolve.ts` (`createGitResolver`, the agent-backed `RebaseResolver` implementation); plus `remote.ts` (fetch with prune, remote default-branch resolution, ls-remote branch-tip listing) and `agent-tools.ts` (shared headless-git-agent tooling — the SideRunner slice, the stage-and-commit `git` allowlist, truncated results, and refusal shaping — used by the commit agent, the rebase resolver, and the skill-evolution proposal agent). The extension consumes them directly and also exposes the high-level operations (`commitAll`, `smartPush`, `smartPull`) to all extensions through the **`app.git`** service (see [DES-001](../design/DES-001-unified-extension-api.md)), so consumers like [projects](projects.md) and [memory](memory.md) use the service rather than importing this extension. What remains under `src/extensions/git/` is purely extension-specific: `scrub.ts` (history rewrite via `git filter-repo`), `guardrail.ts` (compound-command splitting + the destructive deny patterns + the `tool_call` interceptor), `tools.ts`, `processor.ts`, and `hooks.ts`. Commit, sync, and scrub take a `cwd`, so the same primitives serve the workspace, registered projects, and the memory store.
...
| `src/git/commit-agent.ts` *(core module)* | `CommitAgent` type, `createCommitAgent`: a headless side agent that groups changes into cohesive commits via cwd-scoped tools | Two modes — `"workspace"` groups by area, `"project"` matches the repo's own commit style (`git log` + CONTRIBUTING/CLAUDE/AGENTS); the `git` tool is an allowlist (`status`/`diff`/`log`/`add`/`commit`/`show`) so the agent can only add commits; commits run with `core.editor=true`/`commit.gpgsign=false`. `createCommitAgent` exposed via `app.git`; the git extension imports it directly for workspace mode; the tooling lives in `src/git/agent-tools.ts` — one implementation shared with the rebase resolver (`resolve.ts`) and the [skill-evolution](skill-evolution.md) proposal agent |
...
| `src/git/remote.ts` *(core module)* | Remote inspection: `fetchRemote` (fetch origin `--prune`, returns the git result instead of throwing), `resolveRemoteDefaultBranch` (local `origin/HEAD` symbolic ref → `ls-remote --symref` → error), `listRemoteBranchTips` (authoritative branch-name→tip listing for a ref pattern) | `execFile` via `runGit`/`runGitCapture`, no shell strings (DES-002); the throw/result split is deliberate — a failed fetch is a soft-abort signal for the caller, a failed listing is exceptional. Consumers: skill-evolution reconcile/verify, projects' default-branch resolution |
| `src/git/agent-tools.ts` *(core module)* | Shared headless-git-agent tooling: the `AgentRunner` slice, the stage-and-commit `git` subcommand allowlist (`status`/`diff`/`log`/`show`/`add`/`commit`), truncated `textResult` shaping, instructive `refusal`s; `runGitTool` runs commits with `core.editor=true` plus per-consumer config (e.g. `commit.gpgsign=false`) | One implementation behind three scoped-git consumers — the commit agent (`commit-agent.ts`), the rebase resolver (`resolve.ts`), and the skill-evolution proposal agent; per-consumer surface policy (extra argument guards, branch namespaces) stays with the consumer ([DES-010](../design/DES-010-agent-driven-git-host-verified.md)) |
...
- `src/git/remote.ts` exists for the [skill-evolution](skill-evolution.md) reconcile/verify stages (fresh fetch, default-branch resolution, authoritative branch-tip listing) and is also the resolution path behind [projects](projects.md)' default-branch checkout — one chain, no per-consumer variants
- skill-evolution is a third consumer of the scoped-git-tool pattern (agent drives git through an allowlisted tool; the host decides outcomes from git state) — see the workspace commit agent's decision above and [DES-010](../design/DES-010-agent-driven-git-host-verified.md). Its store writes reach the remote through the finalize `git-commit` backstop whether or not the memory extension is enabled
