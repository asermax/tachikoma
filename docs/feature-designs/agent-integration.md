...
`AgentManager` is the single construction path for every pi session. It owns the process-wide `AuthStorage`/`ModelRegistry`/`ModelTiers`, and `open(options)` varies along three axes: persistence (`SessionManager.create` / `.open(sessionFile)` / `.forkFrom(forkFromFile)` / `.inMemory`), binding (all registered factories, the curated background-factory subset, or `bare`), and model (`tier`, or an explicit `model` reference that pins the model over the tier). The core base prompt is `AgentManager`'s own concern: for any non-bare session that brings no explicit `systemPrompt`, it installs `buildMainSystemPrompt({ workspaceRoot })` (identity + guidance + workspace root) as the `systemPromptOverride`, replacing pi's coding prompt; SOUL/USER and per-extension guidance are layered on by extensions via `provideContext` (see [DES-005](../design/DES-005-base-prompt-ownership.md)). `provideContext`'s hidden-message mode (`customType`) injects once per session by default; an optional `options.debounceMs` (or the `provideDebouncedContext` helper, default 30 min) turns that latch into a leading-edge refresh-rate limiter — the first turn injects, further turns within the window are suppressed, and the next turn at/after the boundary re-injects with content re-resolved. This is distinct from [DES-007](../design/DES-007-debounced-background-task.md)'s trailing-edge coalescing debounce (`createDebouncedTask`, which fires once after a burst settles): there is no trailing action to coalesce and no timer to arm, only a per-turn timestamp comparison, so the gate is a pure function (`dueForInjection`) and the window state lives in the per-session factory closure. A fresh `DefaultResourceLoader` is built per open, which is what makes session replacement in the coordinator a plain re-open. `open()` also carries a fourth, opt-in axis — skill loading: `skillPaths` feeds the loader's `additionalSkillPaths` and `forceLoadSkills` binds the force-load injection factory (`src/agent/force-load-skills.ts`), so a headless run can ground itself in specific skills without widening its tool surface or system prompt. On top of `open`, `forkAndContinue(file, prompt, tier, tools?)` is the post-processing fork primitive: a non-bare fork of `file` (persona + history live), tool-restricted by `tools`, handed one follow-up user turn and disposed — used by memory extraction and core-context. Because a non-bare fork binds every registered factory, those forks also fire each factory's `before_agent_start` work; `AgentManager` tracks a `forkDepth` counter around each `forkAndContinue` run and exposes `isForking()` (surfaced as `app.agent.isForking()`) so a factory can scope per-turn work to genuine top-level turns and skip it inside the post-processing forks.
...
| `src/agent/side-run.ts` | `SideRunner.complete/classify/run` | `completeSimple` for one-shots (forwarding `maxTokens`/`temperature`/`signal`); prompt-engineered JSON (string-aware brace extraction) + validation + single retry for classify, which caps output and samples at temperature 0 and forwards an abort signal so a deadline cancels the call; disposable bare session for `run`, which forwards `isolatePrompt` to `open()` for fully isolated delegated prompts, `backgroundExtensions` to bind the background-factory subset while dropping the hard tool allowlist (so the bound tools stay active), and `skillPaths`/`forceLoadSkills` unchanged (see `force-load-skills.ts`) — the skill-loading pair for grounded headless runs |
| `src/agent/force-load-skills.ts` | `forceLoadSkillsFactory({names, log, readSkill?})`: a pi extension factory whose `before_agent_start` handler copies the named skills' bodies from pi's catalog (`stripFrontmatter`-stripped) into one hidden `skill-content` message | Caller-directed skill loading for grounded headless runs (skill-evolution's proposal agent force-loads the authoring guides) — not the classifier-picked proactive injection of the skills extension's `suggest.ts` (the structural precedent; its workflow-start wording is deliberately not carried over); once-per-session latch per name; per-skill fail-soft (absent from catalog or unreadable → warn and skip, empty → debug skip, the run proceeds ungrounded); single-prompt runs only — no `session_compact` re-evaluation |
| `src/agent/skill-body.ts` | `readSkillFile` (default synchronous SKILL.md reader) and `readSkillBody(skill, readSkill, log, label)`: a skill's frontmatter-stripped, trimmed body, or null | The one shared body-load mechanic for both hidden-`skill-content` injectors (`suggest.ts`'s proactive loader and `force-load-skills.ts`), so a pi strip/`filePath` shape change is edited once; each injector keeps its own framing and render tokens; fail-soft per skill (unreadable → warn, empty → debug, caller skips that skill) |
| `AgentManager.forkDepth` / `isForking()` | Tracks whether a `forkAndContinue` run is in flight | Counter incremented as the first statement of `forkAndContinue` and released in `finally` (covers `open` failures and nested/parallel forks); `app.agent.isForking()` lets a factory skip per-turn work inside post-processing forks. `SideRunner` runs (`complete`/`run`) do not touch it |
...
**Choice**: `classify()` appends the JSON Schema to the system prompt, extracts JSON from the reply (fenced block, then a string-aware brace scan to the first balanced object — ignoring trailing prose), validates with the same TypeBox machinery as config parsing (`parseWithSchema`), and retries once with a format reminder (skipping the retry when an abort signal has already fired). Output is capped (`maxTokens`, default 256) and sampled at `temperature` 0, and a caller-supplied `AbortSignal` is forwarded so a deadline cancels the underlying request rather than leaving it racing.
...
- The session-tree seam (`src/agent/session-tree.ts`) wraps the append-only `SessionManager` primitives [ADR-014](../architecture/ADR-014-session-source-of-truth.md) adopted for the trunk model: `branchWithSummary` (collapse + re-seat), `reseatLeaf` (`branch(id)` — move the leaf to an earlier entry), and `appendState`/`appendInContextEntry` (`appendCustomEntry`/`appendCustomMessageEntry`). There is no delete primitive; `/rollback`'s rewind is a `branch(id)` re-seat that leaves the wrong-framing exchange as an inert off-path branch. The verified surface is tracked in `docs/reference/pi-sdk-notes.md` and re-checked on upgrade.
