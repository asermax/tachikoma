...
| `src/agent/side-run.ts` | `SideRunner.complete/classify/run` | `completeSimple` for one-shots; prompt-engineered JSON + validation + single retry for classify; disposable bare session for `run`, which forwards `isolatePrompt` to `open()` for fully isolated delegated prompts |
