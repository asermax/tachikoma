...
trunk close    ──> memory-trunk-close (main phase):
                    extract: for each unextracted branch, fork it (episodic + topics) ──> sweep
...
Each extraction forks the branch (`forkAndContinue`), composes the store's instruction from shared sections, hands the forked assistant one follow-up turn under a file-tool allowlist, then sweeps emptied files. The maintenance phases have no conversation to fork, so they run bare headless side-runs (`SideRunner.run`) over the store on disk, each guarded by an idempotent session-entry marker so a crash re-runs a phase at most once.
...
| `src/extensions/memory/close-pipeline.ts` | `createTrunkClosePipeline` — the phased, marker-guarded trunk-close pipeline (extract → prune → consolidate → core-context) | Pluggable seam: phase bodies reuse `maintenance.ts` logic behind stable exported functions; markers written only after a body completes |
...
| `src/util/dates.ts` | `localIsoDate` (neutral util, used by memory extraction and the context processor) | Local timezone — date-stamped artifacts follow the user's day, not UTC |
