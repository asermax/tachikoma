...
| `src/extensions/memory/close-pipeline.ts` | `createTrunkClosePipeline` — the phased, marker-guarded trunk-close pipeline (extract → prune → consolidate → core-context) | Pluggable seam: phase bodies reuse `maintenance.ts` logic behind stable exported functions; `extractBranches` iterates `EXTRACTION_STORES` (so learnings spawns no fork), runs a branch's forks concurrently and branches serially — `parallelizeExtraction` toggles it and all forks settle before the shared branch file is deleted — and sweeps each fork's `FORK_WRITE_STORES`; markers written only after a body completes; carries `statusLabel: "Processing memories"` (and `transcript-archive` carries "Archiving transcript") so the close surfaces friendly progress on the channel's preparation lead-in; the pipeline also threads the post-processor context's optional `status` callback (populated by the coordinator) to emit an opener naming the trunk's day + branch count, a begin/complete pair per extracted branch, a per-store prune line, and the consolidate/core-context phase starts — routed onto the persistent lifecycle message during a lifecycle close (or the lead-in otherwise), every emission optional-chained so a headless run with no callback emits nothing (the coordinator owns the terminal success/failure line); the branch loop itself delegates to the shared walk (`walkBranches`, `src/sessions/branch-walk.ts`) — memory supplies the extraction body and marker pair and throws when the walk reports failed branches (keep-trunk-unclosed semantics), while the walk owns skip-if-marked, the cut, settle-then-delete, and per-branch isolation; the [skill-evolution](skill-evolution.md) analysis pass is the second consumer |
...
| `src/util/dates.ts` | `localIsoDate(date?, timezone?)` and `formatTimestamp(date, timezone?)` (neutral tz-aware utils, used by memory extraction, the context processor, notifications, process status, and project commit messages) | Local calendar date/timestamp in a given IANA timezone (process timezone when omitted), so date-stamped artifacts follow the user's day, not UTC; `localIsoDate` is the *default* for `storeInstruction`, but the trunk-close pipeline passes the trunk's own day instead of wall-clock |
...
**Consequences**: Pro: one place for all daily memory work; marker-guarded idempotency; a clean slot for DLT-173's richer consolidation; the close surfaces phased/per-branch progress through the coordinator's `status` surface (routed to the lifecycle message during a lifecycle close), so the user sees the pipeline advance rather than only its final outcome. Con: memory work is deferred to trunk close, so a crash before close delays it to recovery; topics maintenance is no richer than before until DLT-173.
...
- The trunk completion-marker union carries two trunk-close kinds: memory's `branchExtracted` (keyed by `branchId`) and [skill-evolution](skill-evolution.md)'s `skillEvolutionAnalyzed` (keyed by `summaryEntryId`, stable under topic renumbering) — one marker mechanism, two payload kinds (see [DES-008](../design/DES-008-marker-computed-effective-state.md)).
