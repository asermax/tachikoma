...
### DLT-175: Daily session with collapsible topic branches
**Status**: ✓ Implementation
**Depends on**: None
**Priority**: 2 (High)
**Complexity**: Hard
**Description**: Replace the topic-based session model — where each topic shift closes the active session and opens a new one — with a single daily persistent session ("trunk") whose topic shifts become collapsible branches. When the topic shifts, the current branch collapses back into the trunk as a generated branch summary (user goal, key decisions, completed work, changed files/commands, open questions, next steps) and a fresh branch forks for the new topic, growing a linear daily trunk of summaries. Topic boundaries are detected by a shadow-fork classifier that forks the live branch into a throwaway headless session and asks the conversation itself whether the incoming message continues the current topic. Prior-topic context is pulled in on demand rather than carried forward: when a new branch resembles a prior one, a pointer to that branch's summary plus its last exchange is injected, and an on-demand branch-lookup tool spins up a headless fork of any prior branch to answer focused questions. Each night the daily session closes and every branch is processed independently for memory extraction. Adopts the pi SDK's native session-tree primitives (branch-with-summary, branched/forked headless sessions, session-context tree traversal) that the current architecture deliberately bypasses, reworking boundary detection, the session registry, context injection, and session-close post-processing. The REPL channel is removed as part of the migration so only the Telegram channel is adapted to the trunk model (Telegram becomes the default; a richer local terminal interface returns later via the pi coding-agent interface). Validated end-to-end via a standalone pi-extension prototype; the daily-session lifecycle and nightly per-branch processing remain to build. Likely supersedes the proactive pre-compaction session handoff (DLT-047) and the fork-to-verify session-routing rollback (DLT-078), and must be reconciled against the parallel-conversation-sessions model (DLT-065) during speccing.
