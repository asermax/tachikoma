...
**Priority**: 4 (Low)
**Complexity**: Hard
**Description**: Enable sharing markdown documents (plans, design notes, project documentation) with collaborators for structured inline feedback. Users share content generated by or stored in Tachikoma, and collaborators add annotations and comments directly on the rendered document — without requiring a separate collaborative editing platform. The initial scope covers rendering markdown to styled HTML and collecting inline comments; the comment format, storage, and notification mechanisms are defined during speccing.
