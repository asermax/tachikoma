...
**Priority**: 3 (Medium)
**Complexity**: Easy
**Description**: Users can customize the assistant's personality and per-user knowledge through SOUL.md and USER.md (plus pi-native AGENTS.md), which the foundational-context system layers onto the system prompt — but there is no way to inject custom behavioral instructions into the core system prompts themselves (the main conversation prompt, the background-task prompt, and the subagent prompt), so global rules like "always respond concisely" or "never notify before 9 AM" can only be added by editing source code. This delta introduces a prompt-layers folder: convention-based, optional plain-markdown files under `.tachikoma/prompts/` that are appended to specific system prompts — `main.md` to the main agent prompt, `background.md` to the background-task prompt, `subagent.md` to the subagent prompt, and `shared.md` to all three (appended after the specific layer). Each layer is appended after the hardcoded base prompt and before the SOUL/USER/AGENTS context that foundational-context injects; missing or empty files are ignored so the mechanism is purely additive. Speccing resolves the read cadence (read once at startup and cached, versus re-read per session to match the existing foundational-context behavior so edits apply without a restart), whether subagents inherit `shared.md` but not `main.md`, and how the currently-constant subagent prompt becomes dynamic to accept a layer. The scope is limited to the loader and injection into the existing prompt builders; no new configuration surface, frontmatter, or templating syntax is introduced.
