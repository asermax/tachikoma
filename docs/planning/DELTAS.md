...
### DLT-181: Temporary side conversations
**Status**: ✗ Defined
**Depends on**: None
**Priority**: 2 (High)
**Complexity**: Hard
**Description**: Let the user open a temporary side conversation inside an active conversation — a quick question or a tangent — then close it and return to the parent conversation with its context intact, like opening a scratch buffer while editing a file. Two modes are supported: a one-turn mode (ask something specific, get an answer, automatically return) and a multi-turn mode (a proper side conversation with its own context that the user explicitly closes to come back). Unlike an automatic topic shift, which collapses the current branch to begin a new topic, a side conversation is explicitly user-initiated and explicitly closed — it parks the parent rather than ending it. Speccing resolves how the side conversation is initiated, how much of the parent's context it sees, what state is restored on return, whether it persists as a resumable branch or stays ephemeral, whether and how a closed side conversation is summarized back into the parent, and where it attaches in the session tree relative to the parent branch.
