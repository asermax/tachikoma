...
**Priority**: 1 (Critical)
**Complexity**: Hard
**Description**: Users can hold multiple conversations and switch between them without losing their place in any of them. When a new message arrives while the assistant is busy and represents a distinct topic, it spawns as a separate session with its own context and history, enabling the user to follow up on something urgent without waiting for a long-running task to complete. Beyond these peer concurrent sessions, the user can also start a side conversation within the current active flow — pausing the parent conversation (held in place rather than collapsed or summarized, so no post-processing or summary fires for it) and resuming it verbatim, with full context intact, when the side conversation closes; the side conversation itself may be a single turn or a proper multi-turn back-and-forth. Where possible, additional conversations should be represented within the pi session tree — as branches structurally connected to the parent — rather than always as wholly separate sessions, so context-switching and verbatim resume reuse the existing tree and branching primitives instead of a parallel session-tracking mechanism. Speccing resolves how a paused-but-not-closed conversation is represented against the coordinator's single-active-session model (a held branch on the session tree versus an explicit session stack), how the parent's full context is preserved across the pause and restored on resume, how a nested side conversation is opened and scoped relative to the daily trunk's append-only branching model and to peer concurrent sessions, whether multiple side conversations can nest or only one is held at a time, the user-facing commands or controls to start and end a side conversation and how they surface per channel, how the side conversation's own post-processing and memory extraction are handled on close, and how starting a side conversation or switching sessions interacts with an in-flight exchange on the parent.
...
### DLT-182: Multi-turn branch channels and side chats
**Status**: ✗ Defined
**Depends on**: None
**Priority**: 2 (High)
**Complexity**: Hard
**Description**: The `ask_branch` tool recovers context from a prior topic branch with a single one-shot question — it forks the branch's original leaf, answers, and disposes the fork — so the agent cannot ask follow-ups or build on a previous answer. This delta replaces it with an explicit open → message → close lifecycle backed by a semi-persistent branch channel whose connection is held between calls rather than re-created each time: `open_branch` opens a channel, `message_branch` sends a message to an opened channel and receives a response (accumulating multi-turn context without re-opening, enabling clarification follow-ups and cross-checking across turns), and `close_branch` closes the channel and releases its state. The channel abstraction serves two patterns. First, multi-turn recovery of a prior collapsed branch: the channel reopens the branch so the agent can ask follow-ups and build on previous answers, replacing `ask_branch`. Second, a side-chat pattern: the agent opens a fresh side conversation in the middle of the current one — a single turn or multi-turn — to explore a subtopic, verify an assumption, or cross-check information without derailing the main conversation, then closes it to return to the main line (optionally folding the result back in as context); this is the agent-side analog of the user's `/checkpoint`/`/back` commands — which park and later resume the main conversation as a tangent — but under agent control and off the live trunk. How the persistent connection is held internally (a long-lived shadow fork versus re-forking with carried context, reusing the non-destructive shadow-fork pattern `ask_branch` already uses), whether `open_branch` takes a prior branch id or can also start a fresh side chat, how a side chat is folded back into the main conversation on close (summary injection like a tangent collapse versus discarded), whether open and close are tied to session lifecycle events, how errors surface (unknown branch, channel already open or busy, connection lost), whether `message_branch` blocks for the response or returns asynchronously, the migration path from `ask_branch`, and how open channels interact with the daily-trunk branching model (a held branch being re-collapsed, or a side chat coexisting with the active trunk) should be evaluated during speccing.
...
### DLT-184: Per-topic model override
**Status**: ✗ Defined
**Depends on**: None
**Priority**: 3 (Medium)
**Complexity**: Medium
...
**Priority**: 3 (Medium)
**Complexity**: Hard
**Description**: Replace the current background-task completion mechanism — an external evaluator that judges each turn as complete/continue/error via a side classifier and nudges the next iteration with its reason — with a goal-driven, self-declaring approach. Background task definitions gain an optional `goal` field, persisted on the task definition and snapshotted onto instances, with a parallel instance-level field for ad-hoc runs and per-run overrides; the field is carried through the create/update/run-now task tools as an optional parameter. When a task runs without a goal, the executor extracts one from the task prompt through a predefined extraction prompt — mirroring the existing side-classifier structured-extraction pattern — and writes it back to the definition so subsequent runs reuse it rather than re-extracting. The agent then works the goal autonomously across iterations and self-declares completion through a new structured completion tool that audits its deliverables against the goal, replacing the external evaluator entirely; the hard iteration limit is retained purely as a safety net. The new completion model, the meaning of the `goal` field, and the recommended goal structure — a measurable end state, a stated check that proves it, and constraints on what must not change — are documented in the tasks usage section injected into the agent's context, in the background system prompt, and in the create/update/run-now tool descriptions. Speccing resolves: the exact continuation and audit prompt wording that enforces conservative self-declaration (re-stating the goal and requiring concrete evidence of the stated check before the agent may call complete), the completion tool's parameters and output and how the instance result is now sourced (the evaluator's reason currently populates the result field), whether the existing evaluator prompt, schema, and fail-open handling are removed or repurposed, how the loop restructures around waiting for a self-declared completion call instead of running a post-turn evaluator, how the goal-extraction step behaves on extraction failure or on already-trivially-satisfied goals, how the `goal` field migrates for existing definitions (lazily on next run versus a backfill pass), whether per-instance goal overrides are supported, and how this interacts with the existing iteration-limit escalation and progress-visibility work on the same subsystem.
