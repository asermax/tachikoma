...
### DLT-185: Persist notifications with restart recovery and retention
**Status**: ✗ Defined
**Depends on**: None
**Priority**: 3 (Medium)
**Complexity**: Hard
**Description**: Notifications live only in memory until delivered: a crash mid-flush-window silently drops unflushed notices, and once delivered a notice survives only as a channel message — nothing records what was sent or what never made it. This delta gives notifications a durable record in the Tachikoma database, superseding the recorded design decision that persisting pending notices was overkill for ephemeral window contents. The notifications extension owns a notifications table recording each emitted notice — payload (source, severity, title, text), the outcome of its delivery, and timestamps — written alongside normal delivery so that losing the database can never block or duplicate one; current routing (flush windows, severity tiers, dedup, digesting) is unchanged and the dedup window stays in memory. Undelivered notices survive restarts: notices persisted before a restart but never delivered are delivered once a channel attaches, exactly once per notification. Rows are kept for a configurable retention period (default one month) and pruned daily — as a contributor to the unified retention sweep pattern if that capability has landed, otherwise by the extension's own scheduled job. Speccing resolves the record granularity (one row per notice versus one per emitted digest delivery); what the delivery outcome captures given that the router hands off to the conversation loop's tiered delivery queue rather than to a channel directly, and when that outcome is stamped back onto the record; how recovery marks a notice delivered so a crash between delivery and marking cannot double-send, and what recovery does when no channel ever attaches; whether recovered notices re-enter through the normal severity-tiered path or a dedicated recovery flush; whether dedup-suppressed notices are recorded; and, once concurrent channels exist, whether recovery targets the first attached channel or every channel.
