...
### DLT-175: Scheduled database backups to Google Drive
**Status**: ✗ Defined
**Depends on**: None
**Priority**: 2 (High)
**Complexity**: Medium
**Description**: Tachikoma's SQLite database (sessions, memory metadata, tasks, workflow state) currently has no backup mechanism and is excluded from the git workspace sync, so a disk failure or a corrupted force-rewrite sync on the host would lose all persistent state with no recovery path. Add a backup extension that, on a configurable schedule (default daily), takes a consistent snapshot of the live database and uploads it to Google Drive, then enforces a configurable retention window (default 30 days) by deleting older backups from the Drive folder. The snapshot must be consistent despite WAL journaling (using the SQLite driver's online backup API or a checkpoint-then-copy) so it can be taken while the agent is running. The extension registers a scheduled job through the scheduler, exposes its cadence, retention period, target Drive folder, and credentials through the typed configuration, and isolates upload/retention failures so a transient Drive or network error logs and retries on the next run rather than crashing the host. The Google Drive authentication model (service-account JSON vs. user OAuth refresh token) and the client approach (official SDK vs. raw REST over fetch) should be evaluated during speccing. Credentials follow the existing plaintext-config pattern for now (as the Telegram bot token does today); migrating them into the planned encrypted token store is a future enhancement, not part of this delta.
