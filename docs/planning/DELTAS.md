...
The original porting roadmap (44 deltas, M1–M5) reached full parity with the Python implementation and was cleared on 2026-06-13; `git log docs/planning/DELTAS.md` preserves the history.

## Deltas

Restored from the Python implementation's backlog (original numbering kept for traceability — it does not relate to the cleared porting-roadmap numbering). Descriptions may still reference Python-era mechanics (MCP servers, Claude SDK, plugin hooks); each delta gets re-specced for the pi stack when picked up.
