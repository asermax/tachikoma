...
`setup` receives an `AppContext` exposing app-level hooks — scheduler, database, session lifecycle, channels, post-processing registration — and can contribute pi extension factories via `app.agent.use((pi) => ...)` for in-session behavior: tools (`pi.registerTool`), event hooks, and context sections (`app.agent.use(provideContext(provide, customType?), { sessionScopes })`, injected as a hidden message or appended to the system prompt).
