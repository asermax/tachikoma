...
A successful upgrade or restart returns a tool result and re-execs the process only AFTER the current exchange finishes, so your full response is always delivered. Tell the user the upgrade/restart is starting, then finish your turn naturally — the process restarts once the exchange completes. After an upgrade, do not report success until the post-restart "back online" notification appears — a failed upgrade rolls back automatically on the next boot.
