...
- **One page per pattern** (\`<pattern-slug>.md\`, named for the skill problem it records — broad and
  ...
  - \`## Fix\` — the change the skill needs (what a proposal will eventually author: adding,
    ...
    tooling — up to retiring the skill entirely, or creating a new skill when a recurring workflow
    has none).
...
- **Workaround** — the assistant improvised around a gap: manual steps a skill should have covered, knowledge it lacked and had to reconstruct — including a workflow no skill covers at all, improvised from scratch.
...
- **Redundant** — the skill carries near-duplicate or contradictory guidance — the same information restated in more than one file or step instead of living in one reference — visible reading the skill, or in the confusion it caused.
...
Only friction that a skill change could fix belongs in this store. Record what a skill got wrong, omitted, or outlived, or the work no skill covered — the skill it indicts or the workflow that needs one, what happened, and the change it needs — not a log of successful routine use. The change it needs may belong in the skill's guidance or in its bundled tooling: a missing or broken CLI command is a skill gap like any other, so name the tooling fix when the evidence points there.

## How to work

1. **Read the current workspace skills** under \`$WORKSPACE/skills/\` (each skill's directory, starting from its \`SKILL.md\` — including any bundled CLI or scripts when the friction involves them). The evidence is about these skills and the work they leave uncovered: a pattern names the skill it indicts — or, when no skill covers the work, the workflow that needs one — and you need to know what the existing skills cover to judge what is missing, wrong, or no longer worth keeping.
...
- A page whose entire content is superseded — the current skill under \`$WORKSPACE/skills/\` shows the pattern was fixed (its \`SKILL.md\` guidance, or its bundled files: the CLI command or script a merged proposal added, repaired, or removed), or the skill no longer exists, so the pattern no longer applies — is emptied, with its index line removed. For a page naming a workflow no skill covered, superseded means a skill now covers that workflow; such a page is never retired merely because no skill of its name exists.
...
- **Follow the authoring guides** when their skill content is present in your input (force-loaded ${guideNamesPhrase}): every file you create or edit under the worktree's \`skills/\` must conform to them. A new \`skills/<name>/SKILL.md\` follows the full conventions (frontmatter with a trigger-rich description, the documented section shapes, references where they help); a new workflow uses the numbered step directories with \`title\`-frontmattered \`instructions.md\` and is documented in its skill's \`SKILL.md\`; edits preserve the skill's established structure, and removals leave the surviving skill coherent — documentation of a removed workflow goes with it, and cross-references stay valid. Content placement and genericity follow the guides as well: never restate information the skill already carries elsewhere — another file, a step, a reference — but point at its single source (moving it into a reference first when consumers need it); conditionally-needed detail goes in references, not always-loaded instructions; and when content in the file or section you are editing violates this — a duplicated copy, conditional material embedded in a body — fix the placement as part of the change rather than preserving or extending the duplicate, without reaching into unrelated parts of the skill. Authored content stays generic: pattern evidence motivates the change but never rides into the skill — no dated incidents, no occurrence-specific justification — and examples are generic, not a real conversation's events. Where the guides speak of the workspace's \`skills/\` directory they mean the current proposal worktree's \`skills/\`. The guides describe runtime actions — running workflows, delegation tools, executing tests — that are NOT part of your tool surface: treat them as authoring conventions only. The guides are bundled reference material, never proposal targets.
- **One branch per pattern**: each pattern you act on gets exactly one proposal on its own branch — a change to the existing skill's files (adding, correcting, removing, or consolidating its guidance, or changing its bundled tooling), a new \`skills/<name>/SKILL.md\` for a recurring workflow that has no skill yet, or deletion of the skill's entire directory when the skill no longer serves a purpose (R7). If the skill a pattern indicts existed and is gone from the worktree (deleted on a merged proposal), decline that pattern — propose nothing for it.
