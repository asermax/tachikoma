...
async def plugins_hook(ctx: BootstrapContext) -> None:
    ...
    Steps:
        ...
        3. Reconcile declared plugins with on-disk state.
        4. Discover loaded plugins.
        5. Construct PluginManager.
        6. Store manager and skill paths in ``ctx.extras``.

    Pre-condition:
        ``ctx.extras["event_bus"]`` must be populated before this hook runs
        (the bus is hoisted to before ``bootstrap.run()`` in ``__main__``).
    """
