...
async def init_submodule(workspace: Path, path: str) -> None:
    ...
    """
    await run_git("submodule", "update", "--init", path, cwd=workspace)
...
async def checkout_branch(submodule_path: Path, branch: str) -> None:
    ...
    Raises:
        RuntimeError: If the command fails.
    """
    await run_git("checkout", branch, cwd=submodule_path)
...
async def fetch(submodule_path: Path) -> None:
    ...
    Raises:
        RuntimeError: If the command fails.
    """
    await run_git("fetch", cwd=submodule_path)
...
async def pull(submodule_path: Path) -> None:
    ...
    Raises:
        RuntimeError: If the command fails.
    """
    await run_git("pull", cwd=submodule_path)
...
async def push(submodule_path: Path) -> None:
    ...
    Raises:
        RuntimeError: If the command fails.
    """
    await run_git("push", cwd=submodule_path)
...
async def add_submodule(workspace: Path, name: str, url: str) -> None:
    ...
    Raises:
        RuntimeError: If the command fails.
    """
    await run_git("submodule", "add", url, f"projects/{name}", cwd=workspace)
...
async def remove_submodule(workspace: Path, name: str) -> None:
    ...
    await run_git("submodule", "deinit", "-f", path, cwd=workspace)
    await run_git("rm", "-f", path, cwd=workspace)
