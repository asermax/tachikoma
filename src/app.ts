...
export const runApp = async (options: RunOptions = {}): Promise<void> => {
  ...
  // Single-instance guard: two processes on one workspace duplicate scheduled actions,
  // conflict over Telegram polling, and corrupt shared trunk/session state. Acquired
  // before the logger and database so a conflict fails fast — the error surfaces via
  // main.ts's console.error startup-failure path (see src/instance-lock.ts).
  const lock = await acquireInstanceLock(workspace.instanceLockFile);
  ...
  try {
    ...
    // Idempotent, and never removes a successor's lock file (see InstanceLock.release).
    await lock.release();
  }
};
